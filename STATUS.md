...
## 2026-09-07 — Propagate settings persistence failures

- `update_settings` previously mutated the live snapshot, attempted persistence through a fire-and-forget `emit_snapshot()`, and returned success even when SQLite rejected the settings write. That violated SPEC §2.3's requirement to show native/core failures as real failures rather than fake success.
- `update_settings_snapshot()` now holds the lifecycle lock, applies a candidate settings patch, persists it through the atomic snapshot transaction, restores the full previous in-memory snapshot on failure, and returns the contextual database error. The Tauri command returns `Result<(), String>`, emits only after durable persistence, and then synchronizes browser policy, tray checks, and startup behavior.
- The red trigger-backed regression failed at the missing helper (`/tmp/dm-settings-persistence-red.log`, `SETTINGS_PERSISTENCE_RED_RC=101`). The fixed regression passed `1/1` and verified both the exact `settings locked` error and the old in-memory/SQLite settings row: `/tmp/dm-settings-command-test.log` (`SETTINGS_COMMAND_TEST_RC=0`). The snapshot atomicity regression also remained green `1/1`: `/tmp/dm-snapshot-command-test.log` (`SNAPSHOT_COMMAND_TEST_RC=0`).
- Rebuilt native verification passed: `settings_ui_persistence_probe.py` saved and restored theme, accent, and density through SQLite and restart; `manager_inline_controls_chromium_probe.py` passed pause/resume/retry, bulk actions, sorting, Add URL, and native snapshot transitions. Evidence: `/tmp/dm-settings-persistence-real.log` (`SETTINGS_REAL_RC=0`) and `/tmp/dm-settings-manager-real.log` (`MANAGER_REAL_RC=0`).
- The final aggregate gate returned zero: Vitest `31` files/`100` tests, TypeScript, frontend and extension builds, Rust `73/73` tests, Cargo build, syntax compilation of all `149` Python fixtures, and `git diff --check`. Evidence: `/tmp/dm-settings-command-gate-vitest.log`, `/tmp/dm-settings-command-gate-tsc.log`, `/tmp/dm-settings-command-gate-build.log`, `/tmp/dm-settings-command-gate-cargo-test.log`, `/tmp/dm-settings-command-gate-cargo-build.log`, `/tmp/dm-settings-command-gate-python.log`, and `/tmp/dm-settings-command-gate-diff.log`. Fixture bytecode was removed and no task-owned Vite/probe process remained. This closes checked settings persistence failure propagation provisionally; the v1 mission remains open.

## 2026-09-07 — Match reference manager density at compact desktop widths

- A fresh Chromium render at the reference-sized `1120x657` viewport showed a concrete UI gap: the manager reserved `312px` for the inspector, leaving long filenames and row metadata truncated while the reference composition used roughly `264px`; `101px` rows exposed only about four and a half rows instead of the dense reference list. The existing responsive probe had no assertion for this target composition.
- The existing `fixtures/responsive_viewport_chromium_probe.py` now captures `/tmp/dm-ui-responsive-reference.png` and asserts, at widths `>=1000`, an inspector no wider than `280px`, row heights no taller than `92px`, and a fully fitting first filename. The pre-fix run failed with inspector `312px`, first-title `clientWidth=204`/`scrollWidth=226`, and `REFERENCE_DENSITY_RED_RC=1`: `/tmp/dm-reference-density-red.log`.
- The generalized CSS now uses a `264px` inspector and `88px` download rows at the desktop and narrow two-column breakpoints. The first green render exposed the inspector tab strip overflowing (`scrollWidth=288` inside `264px`); the focused regression failed as `REFERENCE_DENSITY_TABS_RED_RC=1` in `/tmp/dm-reference-density-tabs-red.log`.
- Inspector tabs now use compact, reference-scale padding/gaps and a `26px` close button. The missing `min-width:0` on the inspector heading’s flex child was also fixed after the next focused regression reproduced a horizontal inspector scrollbar (`scrollWidth=263`/`clientWidth=249`): `/tmp/dm-reference-inspector-scroll-red.log`.
- The final responsive Chromium proof passed all four viewports (`1120x657`, `900x600`, `841x560`, and `780x560`): reference inspector `264px`, rows `88px`, first filename not overflowing, inspector tabs `264/264`, inspector body `249/249`, document/body fully contained, and status footer inside the viewport. Evidence: `/tmp/dm-reference-inspector-scroll-green.log` and `/tmp/dm-ui-responsive-reference.png`.
- Post-change real workflow checks passed: manager inline controls (`/tmp/dm-density-manager-real.log`), extension popup (`/tmp/dm-density-extension-real.log`), native Settings persistence (`/tmp/dm-density-settings-native.log`), and tray feedback `2/2` (`/tmp/dm-density-tray-focused.log`). The native manual probe had one timing-sensitive failure only when run concurrently—the assertion was after the explicit submit click—then passed alone with exactly one request, one provisional job, output `262144` bytes, and SHA-256 `ac6533c30d2d4fcc01be82be68bd63a592d37c49fe769b05aebfb4504fa146b3`: `/tmp/dm-density-manual-native-rerun.log`.
- This closes the checked compact manager layout gap provisionally. No transfer, policy, or provider-specific behavior changed; the v1 mission remains open.

## 2026-09-07 — Contain compact popup and tray surfaces

- Fresh browser visual evidence found a concrete shared-shell defect: the extension mock surface rendered a page scrollbar even though its content fit. At the probe viewport, `documentScrollHeight=1416` while `documentHeight=1400`; the tray surface showed the same margin-collapse pattern at `873` versus `857`. The existing popup probe had no document-containment assertion.
- The new assertion in `fixtures/extension_popup_chromium_probe.py` first reached a fixture-expression syntax error, then reproduced the product overflow as `POPUP_CONTAINMENT_RED2_RC=1`: `/tmp/dm-popup-containment-red2.log`.
- The generalized CSS keeps the 16px top/side inset while removing the redundant bottom margin from both `.popup-surface` and `.tray-surface`, and establishes `#root { display: flow-root; }` so the top margin cannot collapse through the 100vh body. No action or state code changed.
- Live Chromium verification passed: popup document/body containment `1400/1400`, exclusion then re-enable of `example.com`, and Open Manager navigation all passed in `/tmp/dm-popup-containment-green2.log`; tray document/body containment `857/857` and all controls remained visible in `/tmp/dm-next-tray-containment2.png`. Focused Vitest passed popup feedback plus tray feedback: `2` files, `3/3` tests in `/tmp/dm-popup-containment-focused2.log`.
- Native Notifications acceptance remains unclaimed because the existing probe still stops before product interaction at `WebKit inspector did not become ready: [Errno 111] Connection refused` (`/tmp/dm-next-notification-surface.log`). This is the known external boundary, not evidence against the compact surfaces.
- This closes the checked popup/tray document-containment gap provisionally. The v1 mission remains open.

## 2026-09-07 — Align the built extension popup with the shared product surface

- The first fresh real Chromium run against the rebuilt extension reached the actual `chrome-extension://mogdhelapdlmkfgeeaogeehnlclhcnkn/popup.html`, then failed the new visual contract: `.popup-surface`, `.popup-header`, `.product-logo`, and `#open.popup-manager-button` were all absent; the old plain popup body measured `292px`. Evidence: `/tmp/dm-real-extension-red.log` and `/tmp/dm-real-extension-build-red.log` (`PROBE_RC=1`, extension build `0`).
- The cause was structural: the MV3 extension has a separate framework-free HTML entry and did not render the newer React `ExtensionPopup` surface. `extension/popup.html` now uses the shared product hierarchy—branded header and close control, icon-led settings rows, site status card, and the Open Manager action—with inline SVGs matching `src/icons.tsx`. `extension/popup.css` carries the shared popup tokens, typography, spacing, controls, focus treatment, light/dark colors, and contained document geometry.
- `extension/src/popup.ts` keeps the existing Chrome storage/native-message behavior, updates the site-state color class when exclusion changes, and wires the new close control to `window.close()`. No capture, policy, or native-host protocol changed.
- The first green real run passed the visual contract and the existing browser-ownership proof: one browser-owned `49152`-byte file, one source request, matching SHA-256 `bb33c3eac7269e51cea14832f2cc1fdeb718bb5f85cd22fafb15b3aae81ce8a0`, and zero native jobs/processes: `/tmp/dm-real-extension-green1.log`.
- The user-facing visual review then found a real extension-only geometry defect: the hidden status element still displayed because `.form-error { display:flex }` overrode `[hidden]`, adding a blank 10px band; after that fix, fractional text metrics left `0.5px` of document overflow. `extension/popup.css` now explicitly hides `[hidden]` error nodes and uses a 12px bottom margin on the extension’s manager action. Fresh captures `/tmp/dm-built-popup-green1.png`, `/tmp/dm-built-popup-green2.png`, and `/tmp/dm-built-popup-green3.png` show the refinement sequence; the final screenshot has no scrollbar or clipped action.
- Final fresh CDP geometry is recorded in `/tmp/dm-built-popup-geometry-final.log`: viewport/document `500x377`, body `324x376.5`, surface `292x360.5` ending at `y=376.5`, Open Manager bottom `y=363.5`, and `verticalOverflow=false`. The final rebuilt integration probe passed with the same byte/hash proof and visual-node contract: `/tmp/dm-extension-integration-gate.log` (`PROBE_RC=0`).
- Focused verification passed: extension popup tests plus in-app popup feedback `3/3` (`/tmp/dm-extension-focused-gate.log`), root TypeScript `0` (`/tmp/dm-extension-tsc-gate.log`), and extension build `0` (`/tmp/dm-extension-build-gate.log`).
- The complete aggregate gate passed: Vitest `31` files/`100` tests, root `npx tsc -b`, frontend and extension production builds, Rust `73/73` tests, Cargo build, all `149` Python fixture syntax checks without bytecode, and `git diff --check`. Evidence: `/tmp/dm-extension-aggregate-vitest.log`, `/tmp/dm-extension-aggregate-tsc-root.log`, `/tmp/dm-extension-aggregate-build.log`, `/tmp/dm-extension-aggregate-cargo-test.log`, `/tmp/dm-extension-aggregate-cargo-build.log`, `/tmp/dm-extension-aggregate-python.log`, and `/tmp/dm-extension-aggregate-diff.log`. The first explicit TypeScript command was run from `src-tauri` and returned `TS5083`; it was rerun from the repository root and passed, so that wrong-directory result is not acceptance evidence.
- Cleanup removed all four task-owned disposable extension profiles, closed debug ports `9228–9230`, and left no generated fixture bytecode or retained extension Chromium process. Native Notifications acceptance remains unclaimed at the known pre-product WebKit Inspector refusal; the v1 mission remains open and the next pass must choose another reachable horizontal gap.

## 2026-09-07 — Keep narrow Settings navigation labels visible

- Fresh Chromium at the supported `780x560` viewport exposed a real responsive UI gap in the Settings route. The narrow `155px` settings navigation gave `Browser Integration` a `93px` visible span for `109px` of text, so the label rendered as `Browser Inte…`; the other labels fit. Red evidence: `/tmp/dm-settings-nav-red.log`.
- Extending the existing `fixtures/responsive_viewport_chromium_probe.py`—without creating a new harness—now visits General, Downloads, Browser, Network, Notifications, and Appearance at `780x560`. It asserts document/body horizontal containment, settings-pane bounds, and that every settings-nav span's `scrollWidth` fits its `clientWidth`. The first `173px` fix caught the selected-font edge case: active `Browser Integration` measured `111/125`; evidence: `/tmp/dm-settings-nav-green.log`.
- The responsive CSS now gives the settings-only narrow layout a `190px` navigation column (the desktop column is unchanged). This leaves a `398px` content pane while the active label measures `125/125`, and the settings content remains within `x=373..771` of the `780px` viewport.
- Final fresh Chromium evidence passed all four existing manager viewports and all six settings pages. Every page reported document/body `780x560` containment and complete nav labels. The final Browser Integration and Network screenshots show the full selected label, one-column settings content, and reachable lower controls through the internal scroll: `/tmp/dm-settings-nav-green2.log`, `/tmp/dm-ui-settings-responsive-browser.png`, and `/tmp/dm-ui-settings-responsive-network.png`.
- The complete aggregate gate returned zero: Vitest `31` files/`100` tests, root TypeScript, frontend and extension production builds, Rust `73/73` tests, Cargo build, all `149` Python fixture syntax checks without bytecode, and `git diff --check`. Evidence: `/tmp/dm-settings-nav-gate-vitest.log`, `/tmp/dm-settings-nav-gate-tsc.log`, `/tmp/dm-settings-nav-gate-build.log`, `/tmp/dm-settings-nav-gate-cargo-test.log`, `/tmp/dm-settings-nav-gate-cargo-build.log`, `/tmp/dm-settings-nav-gate-python.log`, and `/tmp/dm-settings-nav-gate-diff.log`.
- Cleanup closed the disposable Vite servers on `4173` and `4177`, Chromium debug ports `9231` and `9232`, and removed their three task-owned profiles. This closes the narrow Settings navigation-label gap provisionally; no transfer, policy, native, or provider-specific behavior changed. The v1 mission remains open.

## 2026-09-07 — Preserve navigation semantics and manager selection context

- The existing generalized manager filter/context Chromium probe found two reachable navigation gaps. First, the visually selected `All` filter had no `aria-current`; the red run recorded `selected=true` with `current=null` in `/tmp/dm-nav-current-red.log`. After that semantic fix, the same workflow exposed that visiting Settings replaced the selected `Big Buck Bunny (1080p).mkv` row with the first job when returning to All; the red state-preservation run recorded `ubuntu-24.04-desktop-amd64.iso` instead of the prior selection in `/tmp/dm-nav-state-red.log`.
- `src/App.tsx` now gives the active manager filter and active Settings page `aria-current="page"`, with inactive controls omitting the attribute. The existing selection reconciliation effect skips the synthetic Settings filter, so it still selects the first visible job for real filter changes but preserves the selected download across a Settings round trip.
- The strengthened existing `fixtures/manager_filters_context_chromium_probe.py` does not add a new harness. It waits for actual navigation state, checks one active/current page in both navigation layers, visits General and Browser Integration, returns to All, and asserts the selected job survives before continuing its existing sorting, keyboard selection, context-menu, and removal workflow.
- Real Chromium verification returned `MANAGER-FILTERS-CONTEXT-PROBE: PASS`: all six filter counts/selections, active `aria-current` semantics, Settings navigation semantics, Settings round-trip selection, sorting, row keyboard selection, context-menu targeting, and removal passed. Evidence: `/tmp/dm-nav-state-fixed.log`.
- The root aggregate gate returned zero: Vitest `31` files/`100` tests, root `npx tsc -b`, frontend and extension production builds, Rust `73/73` tests, Cargo build, Python fixture compilation, and `git diff --check`. Evidence: `/tmp/dm-navigation-gate-vitest.log`, `/tmp/dm-navigation-gate-tsc.log`, `/tmp/dm-navigation-gate-build.log`, `/tmp/dm-navigation-gate-cargo-test.log`, `/tmp/dm-navigation-gate-cargo-build.log`, `/tmp/dm-navigation-gate-python.log`, and `/tmp/dm-navigation-gate-diff.log`.
- Cleanup terminated only the disposable Vite process group `1938608`, confirmed no listener remained on `127.0.0.1:4173`, and removed the generated `fixtures/__pycache__/` cache. No shared service or unrelated process was changed. This closes the checked navigation semantics and selection-context gap provisionally; the v1 mission remains open.

## 2026-09-07 — Make Inspector tabs keyboard-addressable

- A fresh Chromium DOM audit found that the Inspector's visible Overview/Network/Media/Files/Log strip was implemented as plain buttons: the tab container, buttons, and content panel all reported no tab semantics or relationship. The focused regression was red with `role=null` where the named tablist was required; the live pre-fix readback is retained in `/tmp/dm-inspector-tabs-before.png`.
- `Inspector` now exposes a named `role="tablist"`, `role="tab"` buttons with `aria-selected`, stable `id` values and `aria-controls`, and one `role="tabpanel"` with `aria-labelledby`. Roving `tabIndex` keeps only the selected tab in the page tab order; ArrowLeft/Right and Home/End change the selected tab and move focus without changing panel content or transfer behavior.
- The focused existing semantics suite returned `3/3` in the green run, covering the two existing menu contracts plus the Inspector tab/panel relationship and ArrowRight activation. Evidence is the `npx vitest run src/menu-semantics.test.tsx` result.
- Real Chromium verification returned the expected tablist/panel structure, `true/false` selection states, `0/-1` roving tab indexes, and focus on Network after an ArrowRight event. The page remained contained at `1120px` with no horizontal or vertical document overflow. Evidence: `/tmp/dm-inspector-tabs-real.log` and `/tmp/dm-inspector-tabs-real.png`.
- The existing generalized manager filter/context workflow remained green after the shared Inspector change: all six filter counts/selections, Settings navigation and selection round trip, sorting, keyboard row selection, context-menu targeting, and removal passed in `/tmp/dm-inspector-manager-real.log`.
- The complete aggregate gate returned zero: Vitest `31` files/`101` tests, root `npx tsc -b`, frontend and extension production builds, Rust `73/73` tests, Cargo build, Python fixture compilation, and `git diff --check`. Evidence: `/tmp/dm-inspector-gate-vitest.log`, `/tmp/dm-inspector-gate-tsc.log`, `/tmp/dm-inspector-gate-build.log`, `/tmp/dm-inspector-gate-cargo-test.log`, `/tmp/dm-inspector-gate-cargo-build.log`, `/tmp/dm-inspector-gate-python.log`, and `/tmp/dm-inspector-gate-diff.log`.
- Cleanup stopped the task-owned Vite and Chromium sessions, confirmed no listener remained on `127.0.0.1:4173` or debug port `9233`, removed the generated fixture bytecode cache, and left no task-owned browser process. This closes the checked Inspector semantics gap provisionally; the v1 mission remains open.

## 2026-09-07 — Distinguish notification dismissal controls

- A fresh in-app Notifications DOM audit found every card close control exposed the same accessible name, `Dismiss`. With completed and failed cards present, keyboard/screen-reader users could not tell which notification a close action would remove. The focused red run returned `['Dismiss', 'Dismiss']` instead of card-specific names.
- `NotificationCard` now keeps the same close action but labels each control with its visible notification identity: `Dismiss <title>: <detail>`. The existing notification suite was extended to cover two distinct cards and updated to assert the descriptive name on the single-card dismissal path. No notification state, retention, or native-toast behavior changed.
- Focused verification passed `src/notifications-surface.test.tsx` at `3/3`. Fresh Chromium readback found two cards with distinct labels for the completed and failed notifications, document/body `1120x514` containment, and no horizontal or vertical overflow. Visual review of `/tmp/dm-notify-green.png` found no card, icon, action, spacing, or containment regression; DOM evidence is `/tmp/dm-notify-green.png` plus the CDP run output.
- The complete aggregate gate returned zero: Vitest `31` files/`102` tests, root `npx tsc -b`, frontend and extension production builds, Rust `73/73` tests, Cargo build, Python fixture compilation, and `git diff --check`. Evidence: `/tmp/dm-notify-gate-vitest.log`, `/tmp/dm-notify-gate-tsc.log`, `/tmp/dm-notify-gate-build.log`, `/tmp/dm-notify-gate-cargo-test.log`, `/tmp/dm-notify-gate-cargo-build.log`, `/tmp/dm-notify-gate-python.log`, and `/tmp/dm-notify-gate-diff.log`.
- Cleanup terminated only the task-owned notification Vite and Chromium sessions, removed `/tmp/dm-notify-profile-1964317` and generated `fixtures/__pycache__/`, and confirmed `4173` and `9234` were no longer listening. This closes the checked notification-label gap provisionally; the v1 mission remains open.

## 2026-09-07 — Keep download-row actions accessible

- A source and live-DOM audit found each download row exposed `role="button"` while containing separate Pause/Resume/Retry and More buttons. The W3C/MDN ARIA guidance for button roles says descendants are made presentational, so those nested controls could land in sequential focus without a corresponding accessible node. The focused red regression returned `role="button"` where a non-masking row group was required.
- `DownloadRow` now uses a labelled `role="group"` with `aria-current="true"` for the selected row and `aria-keyshortcuts="Enter Space"`; its existing focus, click, Enter, and Space selection behavior is unchanged. Pause/Resume/Retry and More remain native labelled buttons. The existing `manager_filters_context_chromium_probe.py` assertion was updated to the new generalized row contract; no new harness was added. Reference: https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/button_role.
- Focused verification passed `src/download-row-accessibility.test.tsx` at `3/3`. Fresh Chromium readback showed `role=group`, `Select project-assets.zip`, `aria-current=true` after Enter, the row retained focus, and both child actions retained their native button names: `/tmp/dm-row-a11y-green.png`. The existing full filter/context/removal workflow also passed in `/tmp/dm-row-a11y-manager-real.log`; visual review found no row spacing, selection, clipping, or action-affordance regression.
- The complete aggregate gate returned zero: Vitest `31` files/`103` tests, root `npx tsc -b`, frontend and extension production builds, Rust `73/73` tests, Cargo build, Python fixture compilation, and `git diff --check`. Evidence: `/tmp/dm-row-a11y-gate-vitest.log`, `/tmp/dm-row-a11y-gate-tsc.log`, `/tmp/dm-row-a11y-gate-build.log`, `/tmp/dm-row-a11y-gate-cargo-test.log`, `/tmp/dm-row-a11y-gate-cargo-build.log`, `/tmp/dm-row-a11y-gate-python.log`, and `/tmp/dm-row-a11y-gate-diff.log`.
- Cleanup terminated only the task-owned row-audit Vite and Chromium sessions, removed `/tmp/dm-row-profile-1969078` and generated `fixtures/__pycache__/`, and confirmed `4173` and `9235` were no longer listening. This closes the checked nested-row semantics gap provisionally; the v1 mission remains open.

## 2026-09-07 — Drive transient menu focus and Escape dismissal

- Follow-up to the menu-semantics slice: Sort, toolbar More, and row context menus exposed roles but never moved focus into the menu, so keyboard users could not reach the items and Escape had no menu-level handler.
- New shared `KeyboardMenu` in `src/App.tsx`: focuses the first item on open, ArrowUp/Down/Home/End move between items, Escape and Tab dismiss, focus returns to the trigger on unmount. Applied to toolbar More, `SortMenu`, and the row context menu. No transfer, policy, or visual change.
- The focused regression was red before the change (no focus target, no dismiss path); the new test covers first-item focus, ArrowDown/End movement, and Escape dismissal.
- Verified: focused `menu-semantics` suite green, full Vitest `31` files/`104` tests, `npx tsc -b` clean, `npm run build:all` green (`/tmp/dm-kbmenu-build.log`), Rust `73/73`, `cargo build` clean (`/tmp/dm-kbmenu-cargo-build.log`), all fixture sources compile, `git diff --check` clean. This closes the checked menu focus-management gap provisionally; the v1 mission remains open.

## 2026-09-07 — Offer native toast actions on Linux (SPEC §14)

- The plugin ceiling is documented, not a version gap: official Tauri docs state "The Actions API is only available on mobile platforms", and the pinned 2.4.0 desktop backend forwards title/body/icon/sound only. No upgrade changes that. Ref: https://v2.tauri.app/plugin/notification/.
- New `src-tauri/src/notify.rs`: pure `actions_for` (completed → Open / Show in folder, failed → View details), Linux delivery via `notify-rust` (the same crate the plugin uses) with a spawned waiter thread, graceful silence with no daemon. Other OSes keep the plain plugin toast until the Windows delivery pass wires WinRT actions. `add_notification` routes through it; `details` shows the manager and emits `notification-action`, which the Manager handles by selecting the job and opening the inspector.
- TDD: Rust mapping tests failed first on the stub (`left: []`), frontend payload test failed first on the missing module; both green after.
- Real daemon-backed proof with dunst under an isolated bus + private Xvfb `:98` (`/tmp/dm-toast-proof.py`): manager booted hidden (`IsUnMapped` via `--startup` + seeded setting), `/status/503` failed honestly, bus monitor saw the Notify carrying `View details` (delivery), `dunstctl action` made the hidden manager `IsViewable` (execution). `TOAST-ACTIONS: PASS` (`/tmp/dm-toast-proof.log`, `/tmp/dm-toast-bus.log`). No XTEST, no inspector; passive reads only.
- Gate: Vitest `32` files/`106` tests, `tsc` clean, both builds, Rust `75/75`, `cargo build`, fixture compile, `diff --check` all zero (`/tmp/dm-toast-gate-*.log`). Private Xvfb `:98` left running for the tray/close probes. Windows WinRT actions stay deferred to the Windows delivery pass.

## 2026-09-07 — Narrow credential handling to what a job still needs (SPEC §16)

- Audit: event messages never embed source URLs and the only stderr log is a DB error string, so no log scrubbing was needed. Two real gaps: `complete_job` kept referrer/post_body/user_agent in the database forever, and reqwest transport errors persist the full request URL (query tokens included) into `job.error`, which also reaches the failure toast.
- `complete_job` now clears referrer/post_body/user_agent. Failed jobs keep theirs: they are still resumable, which SPEC explicitly allows. Cancelled-then-failed rows keep theirs for the same reason; removed rows are gone entirely.
- New `redact_url_credentials`: keeps scheme/host/path, replaces `?...`/`#...` with `[redacted]`, identity elsewhere. All 7 `job.error = Some(error.to_string())` sites route through it.
- Data dir is now `0700` and the database `0600` on every boot (Unix; self-healing for old installs). Windows ACLs and DPAPI/keyring envelope encryption stay deferred to the Windows delivery pass; cookies stay a decided boundary (no cookies permission).
- TDD: three tests failed first (`E0432`, helpers missing), green after. Live proof on the rebuilt binary (`/tmp/dm-cred-proof.py`, isolated HOME, private Xvfb `:98`): real boot wrote `0700`/`0600`; `--capture` with pageUrl/postBody/userAgent then `--commit` completed with all three null in the row; an unroutable `?token=` URL failed with `error sending request for url (http://127.0.0.1:9/nope.bin[redacted])`. `CRED-STORAGE: PASS` (`/tmp/dm-cred-proof.log`).
- Gate: Rust `78/78`, Vitest `32` files/`106` tests, `tsc` clean, `cargo build`, fixture compile, `diff --check` all zero (`/tmp/dm-cred-gate-*.log`; no frontend change, so no rebuild needed).

## 2026-09-07 — Tray toggle decision + close-to-tray without the inspector (SPEC §12)

- Both tray toggle arms inlined their own flip, so the one-policy coherence had no direct coverage. New pure `tray_toggle_next`: each arm flips exactly its flag and keeps the other; both arms now call it. Test failed first (`E0432`), green after. The visual check direction stays source-verified (GUI injection blocked).
...
## 2026-09-08 — Correct the live browser-policy and media regression

- Commit `17c2809` incorrectly removed the extension's `webRequest` media observation while addressing a browser-network failure. That observation is required by SPEC §6.2 to map blob/MSE players to their manifest and segment traffic; without it, X, Reddit, YouTube, and other blob-backed players could only return `Unavailable`. The media response observers, bounded candidate ring, POST form-context observation, and `webRequest` permission are restored.
- The browser-network failure was in policy reconciliation, not media observation. Every frame polled `get-policy`; the worker forwarded every poll to the resident bridge and wrote the same value to extension storage; storage notifications then woke every frame to read again. Policy reconciliation is now one worker-owned, single-flight bridge request per one-second window. Only top-level content frames poll, child frames consume storage updates, and unchanged resident policy is not rewritten.
