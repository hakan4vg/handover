#!/usr/bin/env python3
"""Real Chromium proof of ordinary-download browser fallback.

The native messaging host is intentionally absent.  A trusted click on an
explicit <a download> must therefore be restored through chrome.downloads,
without recursively entering the ordinary-capture path.  The probe uses a
small local HTTP fixture so the browser artifact can be compared byte-for-byte
without contacting an unsafe public source.
"""
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-browser-fallback-chromium-"))
    home = root / "home"
    profile = root / "profile"
    downloads = profile / "Default" / "Downloads"
    home.mkdir(parents=True)
    # Deliberately do not call public.write_native_manifest().  The missing
    # host is the tested failure condition, not an accidental setup omission.
    ...
    try:
        ...
        native_error = native.get("lastError") if isinstance(native, dict) else None
        if not native_error:
            raise RuntimeError(f"native host was unexpectedly available: {diagnostic}")
        ...
        print(
            f"BROWSER-FALLBACK: PASS (browser_bytes={len(browser_bytes)}, sha256={browser_hash}, "
        ...
        print("BROWSER-FALLBACK-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    finally:
...
if __name__ == "__main__":
    try:
        ...
        print(f"BROWSER-FALLBACK-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
