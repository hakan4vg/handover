#!/usr/bin/env python3
"""Real Chromium + Able Player autoplay/start-time audio proof.

The official Able Player audio3 demo loads a top-level audio element with
``data-start-time=140`` and ``autoplay``. The probe verifies that real page
behavior reaches that offset while playing, then verifies the injected Download
button, commits through the resident CLI, and compares native bytes with a
...
MEDIA_PAGE = "https://ableplayer.github.io/ableplayer/demos/audio3.html"
PAGE = MEDIA_PAGE
...
def wait_autoplay_start(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    deadline = time.time() + timeout
    raw = 'null'
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const a=document.querySelector('audio#audio1');const source=a?.currentSrc||a?.src||a?.querySelector('source[src]')?.src||'';"
            "return {audio:!!a,source,currentTime:a?.currentTime||0,duration:a?.duration||0,readyState:a?.readyState||0,paused:a?.paused??true,button:!!document.querySelector('#dm-media-download-button')};})())"
        )
        state = json.loads(raw)
        if (state.get("audio") and state.get("source", "").endswith("/media/smallf.mp3") and state.get("currentTime", 0) >= 140 and state.get("readyState", 0) >= 2 and not state.get("paused") and state.get("button")):
            return state
        time.sleep(0.5)
    raise RuntimeError(f"Able Player autoplay audio did not reach start time/button: {raw}")


def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-ableplayer-autoplay-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        autoplay = wait_autoplay_start(client)
        print("ABLEPLAYER-AUTOPLAY-AUDIO:", json.dumps(autoplay, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-autoplay-smallf.mp3"
