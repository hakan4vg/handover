#!/usr/bin/env python3
"""Real Chromium + Able Player accessible HTML5 video proof.

The official Able Player demo exposes a top-level ``#video1`` with the
library's custom accessibility controls and a finite public WWA MP4. The probe
uses a fresh Chromium profile, the real unpacked extension/native host, a
resident real binary, and browser CDP evidence; it activates the player,
clicks the product's player-bound Download button, commits through the
resident CLI, and compares native bytes with a browser-context reference.
"""
...
VIDEO_PAGE = "https://ableplayer.github.io/ableplayer/demos/video1.html"
PAGE = VIDEO_PAGE
...
def main() -> int:
    ...
    try:
        ...
        if os.environ.get("DM_KEEP_ABLEPLAYER") != "1":
