#!/usr/bin/env python3
"""Real Chromium + dynamically-created Able Player audio proof.

The official Able Player external6 demo starts with no media element. The probe
clicks its real Add audio control, verifies that the page creates ``#audio1``
and initializes Able Player, then verifies the audio player and injected
Download button, commits through the resident CLI, and compares native bytes
with a browser-context reference.
"""
...
MEDIA_PAGE = "https://ableplayer.github.io/ableplayer/demos/external6.html"
PAGE = MEDIA_PAGE
...
def add_dynamic_audio(client: adaptive.EventCDP, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    raw = 'null'
    control = None
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const b=document.querySelector('#addAudio');"
        ...
        time.sleep(0.5)
    if not control:
        raise RuntimeError(f"Able Player dynamic Add audio control missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        state = json.loads(raw)
        if (state.get("audio") and state.get("source", "").endswith("/media/smallf.mp3") and state.get("wrapperWidth", 0) >= 120 and state.get("wrapperHeight", 0) >= 40):
            return {"control": control, "audio": state}
        time.sleep(0.5)
    raise RuntimeError(f"Able Player dynamic audio did not initialize: {raw}")


def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-ableplayer-dynamic-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        dynamic = add_dynamic_audio(client)
        print("ABLEPLAYER-DYNAMIC-AUDIO:", json.dumps(dynamic, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-dynamic-smallf.mp3"
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            reference_url = "https://ableplayer.github.io/ableplayer/media/smallf.mp3"
