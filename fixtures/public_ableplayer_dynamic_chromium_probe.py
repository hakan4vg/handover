#!/usr/bin/env python3
"""Real Chromium + dynamically-created Able Player video proof.

The official Able Player external6 demo starts with no media element. The probe
clicks its real Add video control, verifies that the page creates ``#video1``
and initializes Able Player, then activates the resulting player, clicks the
product's Download button, commits through the resident CLI, and compares
native bytes with a browser-context reference.
"""
...
VIDEO_PAGE = "https://ableplayer.github.io/ableplayer/demos/external6.html"
PAGE = VIDEO_PAGE
...
def add_dynamic_video(client: adaptive.EventCDP, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    raw = 'null'
    control = None
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const b=document.querySelector('#addVideo');"
        ...
        time.sleep(0.5)
    if not control:
        raise RuntimeError(f"Able Player dynamic Add video control missing: {raw}")
    ...
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const v=document.querySelector('video#video1');const source=v?.querySelector('source[src]');"
            "const r=v?.getBoundingClientRect();return {video:!!v,source:source?.src||'',type:source?.type||'',width:r?.width||0,height:r?.height||0,readyState:v?.readyState||0,paused:v?.paused??true,wrapper:document.querySelector('.player-wrapper')?.style.display||''};})())"
        )
        state = json.loads(raw)
        if state.get("video") and state.get("source", "").endswith("/media/wwa.mp4") and state.get("width", 0) >= 120 and state.get("height", 0) >= 40:
            return {"control": control, "video": state}
        time.sleep(0.5)
    raise RuntimeError(f"Able Player dynamic video did not initialize: {raw}")
...
def main() -> int:
    ...
    try:
        ...
        dynamic = add_dynamic_video(client)
        print("ABLEPLAYER-DYNAMIC-VIDEO:", json.dumps(dynamic, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-dynamic-wwa.mp4"
