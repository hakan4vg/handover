#!/usr/bin/env python3
"""Real Chromium + dynamically-exposed Able Player video proof.

The official Able Player external5 demo starts with its audio and video players
hidden by CSS. The probe discovers and clicks the page's real Show players
control with trusted Chromium input, verifies the video becomes visible and
playable, then clicks the product Download button, commits through the resident
CLI, and compares native bytes with a browser-context reference.
"""
...
VIDEO_PAGE = "https://ableplayer.github.io/ableplayer/demos/external5.html"
PAGE = VIDEO_PAGE
...
def reveal_players(client: adaptive.EventCDP, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    raw = 'null'
    control = None
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const b=document.querySelector('#showPlayers');"
            "if(!b)return {button:false};b.scrollIntoView({block:'center'});const r=b.getBoundingClientRect();"
            "return {button:true,text:b.textContent.trim(),x:r.left+r.width/2,y:r.top+r.height/2};})())"
        )
        state = json.loads(raw)
        ...
        time.sleep(0.5)
    if not control:
        raise RuntimeError(f"Able Player Show players control missing: {raw}")
    ...
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const videos=Array.from(document.querySelectorAll('video'));"
            "const audio=Array.from(document.querySelectorAll('audio'));const v=videos[0];const r=v?.getBoundingClientRect();"
            "return {videos:videos.length,audio:audio.length,videoSource:v?.currentSrc||v?.querySelector('source[src]')?.src||'',videoWidth:r?.width||0,videoHeight:r?.height||0,display:Array.from(document.querySelectorAll('div.able')).map(item=>getComputedStyle(item).display)};})())"
        )
        state = json.loads(raw)
        if state.get("videos") and state.get("videoSource", "").endswith("/media/wwa.mp4") and state.get("videoWidth", 0) >= 120 and state.get("videoHeight", 0) >= 40 and all(value != "none" for value in state.get("display", [])):
            return {"control": control, "players": state}
        time.sleep(0.5)
    raise RuntimeError(f"Able Player players did not become visible: {raw}")


def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-ableplayer-hidden-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        revealed = reveal_players(client)
        print("ABLEPLAYER-HIDDEN-REVEAL:", json.dumps(revealed, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-hidden-wwa.mp4"
        ...
        if os.environ.get("DM_KEEP_ABLEPLAYER_HIDDEN") != "1":
