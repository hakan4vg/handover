#!/usr/bin/env python3
"""Real Chromium + Able Player playlist audio proof.

The official Able Player playlist demo constructs one audio element from a
five-item playlist. The probe discovers and trusted-clicks the second playlist
entry, verifies the selected ``php70alpha.mp3`` source, then verifies the
injected Download button, commits through the resident CLI, and compares native
bytes with a browser-context reference.
"""
...
MEDIA_PAGE = "https://ableplayer.github.io/ableplayer/demos/playlist1-audio.html"
PAGE = MEDIA_PAGE
...
def select_playlist_audio(client: adaptive.EventCDP, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    raw = 'null'
    control = None
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const b=document.querySelector('.able-playlist li:nth-of-type(2) button');"
            "if(!b)return {button:false};b.scrollIntoView({block:'center'});const r=b.getBoundingClientRect();"
            "return {button:true,text:b.textContent.trim(),disabled:b.getAttribute('aria-disabled'),x:r.left+r.width/2,y:r.top+r.height/2};})())"
        )
        state = json.loads(raw)
        ...
        time.sleep(0.5)
    if not control:
        raise RuntimeError(f"Able Player playlist item control missing: {raw}")
    ...
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const a=document.querySelector('audio#audio1');const source=a?.querySelector('source[src]');"
            "const r=a?.getBoundingClientRect();const wrapper=a?.closest('.able');const wr=wrapper?.getBoundingClientRect();"
            "return {audio:!!a,source:source?.src||'',type:source?.type||'',width:r?.width||0,height:r?.height||0,readyState:a?.readyState||0,paused:a?.paused??true,wrapperWidth:wr?.width||0,wrapperHeight:wr?.height||0,wrapperDisplay:wrapper?getComputedStyle(wrapper).display:''};})())"
        )
        state = json.loads(raw)
        if (state.get("audio") and state.get("source", "").endswith("/media/php70alpha.mp3") and state.get("wrapperWidth", 0) >= 120 and state.get("wrapperHeight", 0) >= 40):
            return {"control": control, "audio": state}
        time.sleep(0.5)
    raise RuntimeError(f"Able Player playlist audio did not initialize: {raw}")


def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-ableplayer-playlist-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        playlist = select_playlist_audio(client)
        print("ABLEPLAYER-PLAYLIST-AUDIO:", json.dumps(playlist, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-playlist-php70alpha.mp3"
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            reference_url = "https://ableplayer.github.io/ableplayer/media/php70alpha.mp3"
        ...
        if os.environ.get("DM_KEEP_ABLEPLAYER_DYNAMIC") != "1":
