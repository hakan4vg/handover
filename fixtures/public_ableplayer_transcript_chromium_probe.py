#!/usr/bin/env python3
"""Real Chromium + Able Player transcript-initiated video proof.

The official Able Player transcript demo generates an interactive transcript
from its WebVTT tracks. The probe discovers the runtime Transcript control and
seekpoint from the page DOM, activates both with trusted Chromium clicks, then
uses the real extension/native host, commits through the resident CLI, and
compares native bytes with a browser-context reference.
"""
...
VIDEO_PAGE = "https://ableplayer.github.io/ableplayer/demos/video3.html"
PAGE = VIDEO_PAGE
...
def activate_transcript(client: adaptive.EventCDP, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    button = None
    raw = 'null'
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{"
            "const label=item=>[item.textContent,item.getAttribute('aria-label'),item.getAttribute('title')].filter(Boolean).join(' ').toLowerCase();"
            "const item=Array.from(document.querySelectorAll('button,[role=button],a')).find(candidate=>label(candidate).includes('transcript'));"
            "if(!item)return {item:false,seekpoints:document.querySelectorAll('.able-transcript-seekpoint').length};"
            "item.scrollIntoView({block:'center'});const r=item.getBoundingClientRect();"
            "return {item:true,text:item.textContent.trim(),aria:item.getAttribute('aria-label'),title:item.getAttribute('title'),x:r.left+r.width/2,y:r.top+r.height/2};"
            "})())"
        )
        state = json.loads(raw)
        if state.get("item") and state.get("x") is not None:
            button = state
            break
        time.sleep(0.5)
    if not button:
        raise RuntimeError(f"Able Player Transcript control missing: {raw}")
    ...
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{"
            "const item=document.querySelector('.able-transcript-seekpoint');"
            "if(!item)return {item:false};item.scrollIntoView({block:'center'});const r=item.getBoundingClientRect();"
            "return {item:true,text:item.textContent.trim(),start:item.getAttribute('data-start'),end:item.getAttribute('data-end'),x:r.left+r.width/2,y:r.top+r.height/2};"
            "})())"
        )
        state = json.loads(raw)
        if state.get("item") and state.get("x") is not None:
        ...
        time.sleep(0.5)
    if not seekpoint:
        raise RuntimeError(f"Able Player transcript seekpoint missing after control click: {raw}")
    client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": seekpoint["x"], "y": seekpoint["y"], "button": "left", "clickCount": 1, "modifiers": 0})
    client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": seekpoint["x"], "y": seekpoint["y"], "button": "left", "clickCount": 1, "modifiers": 0})
    time.sleep(1)
    return {"transcript_button": button, "seekpoint": seekpoint}


def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-ableplayer-transcript-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        transcript = activate_transcript(client)
        print("ABLEPLAYER-TRANSCRIPT-ACTIVATION:", json.dumps(transcript, sort_keys=True), flush=True)
        ...
        media_name = "ableplayer-transcript-wwa.mp4"
        ...
        if os.environ.get("DM_KEEP_ABLEPLAYER_TRANSCRIPT") != "1":
