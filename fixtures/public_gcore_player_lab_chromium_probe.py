#!/usr/bin/env python3
"""Real Chromium/native proof for the official Gcore Player Lab.

The Player Lab is a distinct public-page initiation path from Gcore's VOD
selector demo. It loads the finite public Sintel preset through its URL form,
uses the real Gcore player, and verifies the browser -> extension -> resident
capture against an independent browser-context HLS reference.
"""
...
PAGE = "https://g-core.github.io/gcore-videoplayer-js/example/player-lab.html"
...
def load_form_point(client) -> dict | None:
    return evaluate_json(client, """(()=>{const item=document.querySelector('#url-form button[type=submit]');if(!item)return null;item.scrollIntoView({block:'center'});const r=item.getBoundingClientRect();return r.width>20&&r.height>20?{x:r.left+r.width/2,y:r.top+r.height/2,text:item.textContent.trim()}:null})()""")
...
def gcore_state(client) -> dict:
    return evaluate_json(client, """(()=>{const v=document.querySelector('#test-player video');const r=v?.getBoundingClientRect();return {src:v?.currentSrc||v?.src||'',readyState:v?.readyState||0,paused:v?.paused??true,currentTime:v?.currentTime||0,duration:Number.isFinite(v?.duration)?v.duration:0,networkState:v?.networkState||0,error:v?.error?{code:v.error.code,message:v.error.message}:null,rect:r?{x:r.x,y:r.y,w:r.width,h:r.height}:null}})()""")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-public-gcore-lab-chromium-", dir="/var/tmp"))
    ...
    try:
        ...
        load = wait_point(client, load_form_point, "Player Lab load form")
        click(client, load)
        print("GCORE-LAB-LOAD-CLICK: trusted CDP click loaded the public Sintel preset", flush=True)
        state_before = wait_video_ready(client)
        print("GCORE-LAB-SOURCE-STATE:", json.dumps(state_before, sort_keys=True), flush=True)
        if state_before.get("readyState", 0) >= 2 and not state_before.get("paused", True):
            pause = wait_point(client, pause_point, "Player Lab Pause control")
            click(client, pause)
            ...
            if not gcore_state(client).get("paused", True):
                raise RuntimeError("Gcore Player Lab Pause gesture did not pause the VOD")
            print("GCORE-LAB-PAUSE-CLICK: trusted CDP click issued", flush=True)
        point = wait_play_control(client)
        click(client, point); print("GCORE-LAB-PLAY-CLICK: trusted CDP click issued", flush=True)
        state, button = wait_playable(client)
        print("GCORE-LAB-PLAYER:", json.dumps(state, sort_keys=True), flush=True)
        print("GCORE-LAB-BUTTON:", json.dumps(button, sort_keys=True), flush=True)
        button_diagnostic = evaluate_json(client, """(()=>{const item=document.querySelector('#dm-media-download-button');if(!item)return {error:'button disappeared'};const r=item.getBoundingClientRect();window.__dmGcoreLabButtonEvent=null;item.addEventListener('click',event=>{window.__dmGcoreLabButtonEvent={isTrusted:event.isTrusted,defaultPrevented:event.defaultPrevented,target:event.target?.id||event.target?.tagName||null};},{capture:true});const top=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return {outer:item.outerHTML,rect:{x:r.x,y:r.y,w:r.width,h:r.height},pointerEvents:getComputedStyle(item).pointerEvents,top:top?.outerHTML?.slice(0,500)||null};})()""")
        print("GCORE-LAB-BUTTON-DIAGNOSTIC:", json.dumps(button_diagnostic, sort_keys=True), flush=True)
        click(client, button)
        time.sleep(0.5)
        print("GCORE-LAB-BUTTON-EVENT:", json.dumps(evaluate_json(client, "JSON.stringify(window.__dmGcoreLabButtonEvent)"), sort_keys=True), flush=True)
        ...
        print("GCORE-LAB-JOB:", json.dumps({"id":created.get("id"),"media":created.get("media"),"kind":created.get("kind"),"source":redact_url(source)}, sort_keys=True), flush=True)
        browser_reference = browser_source_reference(chrome_port, source)
        print("GCORE-LAB-BROWSER-SOURCE-REFERENCE:", json.dumps({**browser_reference,"url":redact_url(source)}, sort_keys=True), flush=True)
        assert browser_reference.get("hls") is True and browser_reference.get("bytes", 0) > 0, browser_reference
        reference, hls_meta = hls_reference(source, root); print("GCORE-LAB-HLS-REFERENCE:", json.dumps(hls_meta, sort_keys=True), flush=True)
        destination = managed_dir / "gcore-lab-capture.mp4"; public.commit_via_cli(str(home), inspector_port, created["id"], destination.name, str(destination))
        ...
        print(f"GCORE-LAB: PASS (output_bytes={len(output)}, output_sha256={output_hash}, browser_source_bytes={browser_reference['bytes']}, browser_source_sha256={browser_reference['sha256']}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("PUBLIC-GCORE-PLAYER-LAB-CHROMIUM-PROBE: PASS", flush=True); return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_PUBLIC_GCORE_LAB") != "1": shutil.rmtree(root, ignore_errors=True)
