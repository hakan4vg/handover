#!/usr/bin/env python3
"""Real Chromium + HTML5 demo top-level Ogg player media proof.

The public HTML5 demonstration page uses three separate top-level ``<video>``
elements. The first is an Ogg ``windowsill.ogv`` resource, served as
``video/ogg``. The probe uses a fresh Chromium profile, the real unpacked
extension/native host, a resident real binary, and browser CDP evidence; it
selects the Ogg element, scrolls it into view, clicks the product's
player-bound Download button, commits through the resident CLI, and compares
the native result with browser-side bytes.
"""
...
VIDEO_PAGE = "https://html5demo.yo.fr/demo/video.php"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"HTML5 demo Ogg capture created no native media job: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            raw = client.evaluate(
                ...
                "const v=media.find(item=>sourceOf(item).toLowerCase().includes('.ogv')&&visible(item))||media.find(item=>sourceOf(item).toLowerCase().includes('.ogv'))||media.find(item=>sourceOf(item)&&visible(item))||media.find(item=>sourceOf(item))||media[0];"
            ...
            last_raw = repr(raw)
            last = json.loads(raw)
            if last.get("videoCount", 0) and last.get("src", "").lower().endswith(".ogv") and last.get("readyState", 0) >= 2 and not last.get("paused") and last.get("button"):
                return last
    ...
    raise RuntimeError(f"HTML5 demo Ogg player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")


def activate_player(client: adaptive.EventCDP, timeout: float = 90.0) -> None:
    ...
    while time.time() < deadline:
        raw = client.evaluate(
            ...
            "const v=media.find(item=>sourceOf(item).toLowerCase().endsWith('.ogv'));"
            "if(!v)return null;v.scrollIntoView({block:'center'});"
            "const r=v.getBoundingClientRect();"
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"HTML5 demo Ogg player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"HTML5 demo Ogg player did not instantiate after surface click: {raw}")
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if "html5demo.yo.fr" not in host:
            continue
        if not urlsplit(url).path.lower().endswith((".mp4", ".webm", ".ogg", ".ogv", ".mp3", ".wav")):
            continue
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-html5demo-ogg-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("HTML5DEMO-OGG-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("HTML5DEMO-OGG-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("HTML5DEMO-OGG-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".media"
        media_name = "html5demo-windowsill" + suffix
        managed = managed_dir / media_name
        print("HTML5DEMO-OGG-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("HTML5DEMO-OGG-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            ...
            print("BROWSER-HTML5DEMO-OGG-REFERENCE:", json.dumps({"url": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("HTML5DEMO-OGG-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"HTML5DEMO-OGG-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("HTML5DEMO-OGG-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_HTML5DEMO_OGG") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"HTML5DEMO-OGG-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
