#!/usr/bin/env python3
"""Real Chromium + Iframe Tester Paciello audio public-player media initiation proof.

The official Iframe Tester Paciello audio demo uses the documented HTML5 ``<source>`` form and a
...
VIDEO_PAGE = "https://iframe.verekia.com/?url=https://thepaciellogroup.github.io/AT-browser-tests/acc-name-test/audio.html"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, source: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"Iframe Tester Paciello audio capture created no native media job for {source!r}: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            client.evaluate("Array.from(document.querySelectorAll('iframe')).filter(frame=>frame.src.includes('thepaciellogroup.github.io')).forEach(frame=>frame.scrollIntoView({block:'center'}));'scrolled'")
            iframes = top_iframe_rects(client)
            for context_id, context in list(contexts.items()):
                frame_id = context["frameId"]
                frame = frames.get(frame_id, {})
                frame_url = frame.get("url", "")
                if not frame_url.startswith("https://thepaciellogroup.github.io/"):
                    continue
        ...
        time.sleep(0.5)
    raise RuntimeError(f"Iframe Tester Paciello audio player did not become playable/injected: last={last}; error={last_error}; contexts={contexts}; frames={frames}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"Iframe Tester Paciello audio player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"Iframe Tester Paciello audio player did not instantiate after surface click: {raw}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-iframe-paciello-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        assert player["frameUrl"].startswith("https://thepaciellogroup.github.io/"), player
        ...
        media_name = "iframe-paciello-audio" + suffix
        ...
        if os.environ.get("DM_KEEP_IFRAME_PACIELLO_AUDIO") != "1":
