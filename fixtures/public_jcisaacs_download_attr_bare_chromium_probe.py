#!/usr/bin/env python3
"""Real Chromium/native proof for a public bare <a download> anchor.

The jcisaacs HTML5 Download Attribute page contains a real bare-download
anchor (`download` attribute with no value, same-origin href=samp/htmldoc.html).
A clean profile records the browser-owned download named from the URL basename.
A fresh resident+extension profile trusted-clicks that anchor and proves the
extension's document-capture path creates one native job named from the URL
basename (htmldoc.html) with byte/hash-identical output.
"""
...
def wait_page(client, timeout: float = 45.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            last = evaluate_json(client, "{href:location.href,state:document.readyState,anchors:document.querySelectorAll(\"a[href*='htmldoc.html'][download]\").length}")
            if last.get("state") == "complete" and last.get("anchors", 0) >= 1:
                return last
        except Exception:
            pass
        time.sleep(0.4)
...
def anchor(client) -> dict:
    point = evaluate_json(
        client,
        """(()=>{const all=document.querySelectorAll('a[href*="htmldoc.html"][download]'); const a=all[0]; if(!a)return {error:'bare download anchor missing'}; a.scrollIntoView({block:'center'}); const r=a.getBoundingClientRect(); return {href:a.href,rawHref:a.getAttribute('href'),download:a.getAttribute('download'),text:a.textContent.trim(),x:r.left+r.width/2,y:r.top+r.height/2};})()""",
    )
    if point.get("error"):
        raise RuntimeError(point["error"])
    if point["href"] != EXPECTED_SOURCE or point.get("download") != "":
...
def native_run(root: Path, reference: dict) -> None:
    ...
    try:
        ...
        point = anchor(client)
        trusted_click(client, point)
        print("TRUSTED-BARE-DOWNLOAD-CLICK:", json.dumps(point, sort_keys=True), flush=True)
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.2)
        if job is None:
            raise RuntimeError(f"native job missing for bare download anchor: {public.jobs(db)}")
        ...
        print(f"JCISAACS-BARE-DOWNLOAD-ATTR: PASS (page={PAGE}, source={EXPECTED_SOURCE}, filename={EXPECTED_NAME}, browser_bytes={reference['size']}, browser_sha256={reference['sha256']}, native_bytes={native_size}, native_sha256={native_hash}, jobs={len(public.jobs(db))}, browser_downloads={browser_files})", flush=True)
        print("JCISAACS-BARE-DOWNLOAD-ATTR-PROBE: PASS", flush=True)
    except Exception:
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-jcisaacs-bare-"))
    xvfb, display = hls.start_xvfb()
    support.DISPLAY = display
    try:
        ...
        if os.environ.get("DM_KEEP_JCISAACS_BARE") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"JCISAACS-BARE-DOWNLOAD-ATTR-PROBE: FAIL: {error}", flush=True)
        raise