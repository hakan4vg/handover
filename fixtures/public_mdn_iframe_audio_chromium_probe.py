#!/usr/bin/env python3
"""Real Chromium + MDN iframe audio public-player media initiation proof.

The official MDN iframe audio demo uses the documented HTML5 ``<source>`` form and a
...
VIDEO_PAGE = "https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/audio"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, source: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"MDN iframe audio capture created no native media job for {source!r}: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            client.evaluate("Array.from(document.querySelectorAll('iframe')).filter(frame=>frame.src.includes('interactive-examples.mdn.mozilla.net')).forEach(frame=>frame.scrollIntoView({block:'center'}));'scrolled'")
            iframes = top_iframe_rects(client)
            for context_id, context in list(contexts.items()):
                frame_id = context["frameId"]
                frame = frames.get(frame_id, {})
                frame_url = frame.get("url", "")
                if not frame_url.startswith("https://interactive-examples.mdn.mozilla.net/"):
                    continue
        ...
        time.sleep(0.5)
    raise RuntimeError(f"MDN iframe audio player did not become playable/injected: last={last}; error={last_error}; contexts={contexts}; frames={frames}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"MDN iframe audio player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"MDN iframe audio player did not instantiate after surface click: {raw}")
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if "interactive-examples.mdn.mozilla.net" not in host and "developer.mozilla.org" not in host:
            continue
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-mdn-iframe-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        assert player["frameUrl"].startswith("https://interactive-examples.mdn.mozilla.net/"), player
        ...
        media_name = "mdn-iframe-audio" + suffix
        ...
        if os.environ.get("DM_KEEP_MDN_IFRAME_AUDIO") != "1":
