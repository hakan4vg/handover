#!/usr/bin/env python3
"""Real Chromium + MDN top-level WebM player media proof.

The official MDN learning-area example uses a top-level HTML5 ``<video>``
with a WebM source and a fallback download link. It uses a fresh Chromium
profile, the real unpacked extension/native host, a resident real binary, and
the fixture records browser network evidence without persisting query values,
clicks the product's player-bound Download button, commits through the
resident single-instance CLI, and compares the native result with browser-side
bytes.
"""
...
VIDEO_PAGE = "https://mdn.github.io/learning-area/html/multimedia-and-embedding/video-and-audio-content/simple-video.html"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"MDN WebM player capture created no native media job: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    raise RuntimeError(f"MDN WebM player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"MDN WebM player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"MDN WebM player did not instantiate after surface click: {raw}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-mdn-webm-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("MDN-WEBM-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("MDN-WEBM-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("MDN-WEBM-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".media"
        media_name = "mdn-rabbit320" + suffix
        managed = managed_dir / media_name
        print("MDN-WEBM-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("MDN-WEBM-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            ...
            print("BROWSER-MDN-WEBM-REFERENCE:", json.dumps({"url": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("MDN-WEBM-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"MDN-WEBM-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("MDN-WEBM-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_MDN_WEBM") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"MDN-WEBM-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
