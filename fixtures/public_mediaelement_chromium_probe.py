#!/usr/bin/env python3
"""Real Chromium + MediaElement.js custom HTML5 player media proof.

The official MediaElement.js homepage exposes a top-level ``#player1`` video
with the library's custom controls and a GitHub-hosted Big Buck Bunny MP4.
The probe uses a fresh Chromium profile, the real unpacked extension/native
host, a resident real binary, and browser CDP evidence; it activates the
player, clicks the product's player-bound Download button, commits through the
resident CLI, and compares the native result with browser-side bytes.
"""
...
VIDEO_PAGE = "https://mediaelement.github.io/mediaelement/"
PAGE = VIDEO_PAGE
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-mediaelement-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("MEDIAELEMENT-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("MEDIAELEMENT-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("MEDIAELEMENT-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".media"
        media_name = "mediaelement-big-buck-bunny" + suffix
        managed = managed_dir / media_name
        print("MEDIAELEMENT-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("MEDIAELEMENT-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            reference_url = "https://raw.githubusercontent.com/mediaelement/mediaelement-files/refs/heads/master/big_buck_bunny.mp4"
            client.call("Page.navigate", {"url": "https://raw.githubusercontent.com/"})
            time.sleep(2)
            browser_reference_result = browser_reference(client, reference_url)
            print("BROWSER-MEDIAELEMENT-REFERENCE:", json.dumps({"url": redacted_url(reference_url), "job_source": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("MEDIAELEMENT-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"MEDIAELEMENT-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("MEDIAELEMENT-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_MEDIAELEMENT") != "1":
