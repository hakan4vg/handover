#!/usr/bin/env python3
"""Real Chromium + MediaElement.js DASH player media proof.

The official MediaElement.js homepage exposes a top-level ``#player1`` video
and a source selector that calls the library's real ``setSrc``/``load`` path.
The probe chooses the page's live DASH option, uses a fresh Chromium profile,
the real unpacked extension/native host, browser CDP traffic, a resident real
binary, and a product player-bound Download click. It requires a completed,
non-provisional native container with video/audio streams and no browser file.
"""
...
DASH_SOURCE = "https://www.bok.net/dash/tears_of_steel/cleartext/stream.mpd"
VIDEO_PAGE = "https://www.mediaelementjs.com/"
PAGE = VIDEO_PAGE
...
def select_dash_source(client: adaptive.EventCDP) -> dict:
    raw = client.evaluate(
        "JSON.stringify((()=>{"
        "const select=document.querySelector('#player1-sources');"
        "const option=Array.from(select?.options||[]).find(item=>{"
        "const value=item.value||'';return value.toLowerCase().split('?')[0].endsWith('.mpd');});"
        "if(!select||!option)return {ok:false,select:!!select,option:false};"
        "select.value=option.value;"
        "select.dispatchEvent(new Event('change',{bubbles:true}));"
        "return {ok:true,value:select.value,url:new URL(option.value,location.href).href,text:option.textContent.trim()};"
        "})())"
    )
    result = json.loads(raw)
    if not result.get("ok"):
        raise RuntimeError(f"MediaElement DASH source option missing: {result}")
    if result["url"] != DASH_SOURCE:
        raise RuntimeError(f"unexpected MediaElement DASH source: {result}")
    return result
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if host != "www.bok.net":
            continue
        if not urlsplit(url).path.lower().endswith((".mpd", ".m4f", ".mp4")):
            continue
...
def wait_for_traffic(client: adaptive.EventCDP, timeout: float = 20.0) -> list[dict]:
    ...
    while time.time() < deadline:
        ...
        if any(item["url"].lower().split("?")[0].endswith(".mpd") for item in records) and any(item["url"].lower().split("?")[0].endswith(".m4f") for item in records):
            return records
        time.sleep(0.5)
    return records
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-mediaelement-dash-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        selected = select_dash_source(client)
        print("MEDIAELEMENT-DASH-SOURCE:", json.dumps(selected, sort_keys=True), flush=True)
        activate_player(client)
        player = wait_player(client)
        print("MEDIAELEMENT-DASH-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("MEDIAELEMENT-DASH-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("MEDIAELEMENT-DASH-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        media_name = "mediaelement-dash-tears-of-steel.mp4"
        managed = managed_dir / media_name
        print("MEDIAELEMENT-DASH-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("MEDIAELEMENT-DASH-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        browser_manifest_reference = None
        if job.get("source", "").startswith(("http://", "https://")):
            client.call("Page.navigate", {"url": "https://www.bok.net/"})
            time.sleep(2)
            browser_manifest_reference = browser_reference(client, DASH_SOURCE)
            print("BROWSER-MEDIAELEMENT-DASH-MANIFEST-REFERENCE:", json.dumps({"url": redacted_url(DASH_SOURCE), **browser_manifest_reference}, sort_keys=True), flush=True)
            if int(browser_manifest_reference["size"]) != 1913:
                raise RuntimeError(f"unexpected browser manifest size: {browser_manifest_reference}")
        ...
        print("MEDIAELEMENT-DASH-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "ffprobe": ffprobe, "manifest_reference": browser_manifest_reference, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"MEDIAELEMENT-DASH-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("MEDIAELEMENT-DASH-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_MEDIAELEMENT_DASH") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"MEDIAELEMENT-DASH-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
