...
VIDEO_PAGE = "https://iandevlin.github.io/mdn/video-player/"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"Mozilla HTML5 player capture created no native media job: {last}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-mozilla-html5-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("MOZILLA-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("MOZILLA-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("MOZILLA-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".media"
        media_name = "mozilla-tears-of-steel" + suffix
        managed = managed_dir / media_name
        print("MOZILLA-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("MOZILLA-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            ...
            print("BROWSER-MOZILLA-REFERENCE:", json.dumps({"url": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("MOZILLA-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"MOZILLA-HTML5-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("MOZILLA-HTML5-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_MOZILLA") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"MOZILLA-HTML5-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
