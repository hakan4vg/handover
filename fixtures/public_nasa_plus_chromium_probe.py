#!/usr/bin/env python3
"""Real public player: NASA+ Video.js/VHS page -> DM button -> capture.

NASA+'s public Astro Smile page opens a Video.js player modal whose top-level
video carries a finite HLS manifest in data-video-url. The probe activates the
page's real Open Video Player control, starts playback with a trusted play
click, clicks the REAL extension media button, and commits through the real
provisional-to-completed flow. Reference = an independent HLS fetch and mux of
the exact source URL the job captured. Asserts byte/hash equality, exactly one
job, empty Chromium Downloads.
"""
...
PAGE = "https://plus.nasa.gov/video/astro-smile/"
...
def wait_open_player(client, timeout: float = 90.0) -> dict:
    deadline = time.time() + timeout
    last = None
    while time.time() < deadline:
        raw = client.evaluate("""JSON.stringify((()=>{const items=[...document.querySelectorAll('button,[role="button"]')];const item=items.find(x=>/^open video player$/i.test((x.getAttribute('aria-label')||x.textContent||'').trim()));if(!item)return null;item.scrollIntoView({block:'center'});const r=item.getBoundingClientRect();return r.width>20&&r.height>20?{x:r.left+r.width/2,y:r.top+r.height/2,text:(item.getAttribute('aria-label')||item.textContent||'').trim()}:null})())""")
        last = json.loads(raw) if isinstance(raw, str) else raw
        if last:
            return last
        time.sleep(1.0)
    raise RuntimeError(f"NASA+ Open Video Player control did not mount: {last}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-nasa_plus-button-", dir="/var/tmp"))
    home = root / "home"
    ...
    try:
        ...
        time.sleep(8.0)
        open_point = wait_open_player(client)
        print("NASA_PLUS-OPEN-CLICK:", json.dumps(open_point, sort_keys=True), flush=True)
        click(client, open_point)
        time.sleep(3.0)
        ...
        while time.time() < deadline:
            ...
            time.sleep(1.0)
        print("NASA_PLUS-STATE:", json.dumps(state, sort_keys=True), flush=True)
        if point is None:
        ...
        print("NASA_PLUS-JOB:", json.dumps({"id": created["id"], "media": created.get("media"), "kind": created.get("kind"), "source": source}, sort_keys=True), flush=True)
        assert created.get("media") is True and source and not source.startswith("blob:"), created
        print("NASA_PLUS-REFERENCE-FETCH: resolving manifest + muxing reference...", flush=True)
        ...
        print(f"NASA_PLUS-REFERENCE: fragments={len(track_urls)} bytes={len(reference)} sha256={reference_hash}", flush=True)
        managed = managed_dir / "nasa_plus-capture.mp4"
        public.commit_via_cli(str(home), inspector_port, created["id"], "nasa_plus-capture.mp4", str(managed))
        ...
        print(f"NASA_PLUS: PASS (output_bytes={len(output)}, output_sha256={output_hash}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("NASA_PLUS-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_PUBLIC_NASA_PLUS") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"NASA_PLUS-PROBE: FAIL: {error}", flush=True)
        raise
