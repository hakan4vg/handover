#!/usr/bin/env python3
"""Real public player: NFB trailer Video.js/HLS embed -> DM button -> capture.

NFB's lawful public trailer embed exposes a finite signed CloudFront HLS
manifest to a Video.js player. The probe starts playback with a trusted play
click, clicks the REAL extension media button, and commits through the real
provisional-to-completed flow. Signed query strings are retained only in
process memory and redacted from diagnostics. Reference = independent HLS
fetch and mux of the exact captured source. Asserts byte/hash equality,
...
PAGE = "https://www.nfb.ca/film/wavemakers/trailer/wavemakers_trailer/embed/player/?iframe_redirect_url=/film/wavemakers/&play=true&context_type=trailer&embed_mode=0&auto_play=true&back_url=/film/wavemakers/"
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-nfb_trailer-button-", dir="/var/tmp"))
    home = root / "home"
    ...
    try:
        ...
        print("NFB-PLAY-CLICK: issued", flush=True)
        ...
        while time.time() < deadline:
            ...
            time.sleep(1.0)
        print("NFB-STATE:", json.dumps(state, sort_keys=True), flush=True)
        if point is None:
        ...
        print("NFB-JOB:", json.dumps({"id": created["id"], "media": created.get("media"), "kind": created.get("kind"), "source": redact_url(source or "")}, sort_keys=True), flush=True)
        assert created.get("media") is True and source and not source.startswith("blob:"), created
        print("NFB-REFERENCE-FETCH: resolving manifest + muxing reference...", flush=True)
        ...
        print(f"NFB-REFERENCE: fragments={len(track_urls)} bytes={len(reference)} sha256={reference_hash}", flush=True)
        managed = managed_dir / "nfb_trailer-capture.mp4"
        public.commit_via_cli(str(home), inspector_port, created["id"], "nfb_trailer-capture.mp4", str(managed))
        ...
        print(f"NFB: PASS (output_bytes={len(output)}, output_sha256={output_hash}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("NFB-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_PUBLIC_NFB") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"NFB-PROBE: FAIL: {error}", flush=True)
        raise
