#!/usr/bin/env python3
"""Real Chromium + Paciello keyboard media initiation proof.

The official Paciello audio demo uses the documented HTML5 ``<source>`` form and a
...
VIDEO_PAGE = "https://thepaciellogroup.github.io/AT-browser-tests/acc-name-test/audio.html"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"Paciello audio capture created no native media job: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    raise RuntimeError(f"Paciello audio player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"Paciello audio player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"Paciello audio player did not instantiate after surface click: {raw}")
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if "thepaciellogroup.github.io" not in host:
            continue
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-paciello-keyboard-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("KEYBOARD-AUDIO-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("KEYBOARD-AUDIO-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        keyboard_state = json.loads(client.evaluate(
            "JSON.stringify((()=>{const b=document.querySelector('#dm-media-download-button');"
            "if(!b)throw Error('Download button missing');window.__dmKeyboardClick=null;"
            "b.addEventListener('click',event=>{window.__dmKeyboardClick={isTrusted:event.isTrusted,defaultPrevented:event.defaultPrevented};},{capture:true,once:true});"
            "b.focus();return {focused:document.activeElement===b,aria:b.getAttribute('aria-label'),text:b.textContent};})())"
        ))
        print("KEYBOARD-BUTTON:", json.dumps(keyboard_state, sort_keys=True), flush=True)
        assert keyboard_state["focused"] is True, keyboard_state
        client.call("Input.dispatchKeyEvent", {"type": "rawKeyDown", "key": " ", "code": "Space", "windowsVirtualKeyCode": 32, "nativeVirtualKeyCode": 32})
        client.call("Input.dispatchKeyEvent", {"type": "keyUp", "key": " ", "code": "Space", "windowsVirtualKeyCode": 32, "nativeVirtualKeyCode": 32})
        deadline = time.time() + 10
        keyboard_click = None
        while time.time() < deadline:
            keyboard_click = json.loads(client.evaluate("JSON.stringify(window.__dmKeyboardClick)"))
            if keyboard_click:
                break
            time.sleep(0.2)
        assert keyboard_click and keyboard_click["isTrusted"] is True, keyboard_click
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".audio"
        media_name = "paciello-keyboard-audio" + suffix
        managed = managed_dir / media_name
        print("KEYBOARD-AUDIO-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("KEYBOARD-AUDIO-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        print("KEYBOARD-AUDIO-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"KEYBOARD-AUDIO-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("KEYBOARD-AUDIO-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_AUDIO") != "1":
