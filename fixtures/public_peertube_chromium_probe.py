#!/usr/bin/env python3
"""Real Chromium/native proof for a public PeerTube watch page.

PeerTube's federated public watch page exposes a real HLS or progressive media
...
PAGE = "https://peertube.tv/w/sQDpFLkMpWcm33kjTKHPYy"
...
def wait_page(client, timeout: float = 90.0) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"PEERTUBE page did not load: {last}")
...
def pause_point(client) -> dict | None:
    return evaluate_json(client, """(()=>{const candidates=[...document.querySelectorAll('button,[role=\"button\"]')].filter(item=>{const text=(item.getAttribute('aria-label')||item.getAttribute('title')||item.textContent||'').trim();const r=item.getBoundingClientRect();return /^pause$/i.test(text)&&r.width>20&&r.height>20&&r.bottom>0&&r.right>0;});const item=candidates[0];if(!item)return null;const r=item.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2,text:(item.getAttribute('aria-label')||item.textContent||'').trim()};})()""")
...
def wait_play_control(client, timeout: float = 120.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        time.sleep(1.0)
    raise RuntimeError(f"PEERTUBE play control did not mount: {last}")
...
def peertube_state(client) -> dict:
    return evaluate_json(client, """(()=>{const v=document.querySelector('video');const r=v?.getBoundingClientRect();return {src:v?.currentSrc||v?.src||'',readyState:v?.readyState||0,paused:v?.paused??true,currentTime:v?.currentTime||0,duration:Number.isFinite(v?.duration)?v.duration:0,networkState:v?.networkState||0,error:v?.error?{code:v.error.code,message:v.error.message}:null,rect:r?{x:r.x,y:r.y,w:r.width,h:r.height}:null}})()""")
...
def wait_video_ready(client, timeout: float = 120.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            last = peertube_state(client)
            if last.get("src") and last.get("readyState", 0) >= 2:
                return last
        except Exception as error:
            last = {"error": str(error)[:160]}
        time.sleep(1.0)
    raise RuntimeError(f"PeerTube video did not become ready: {last}")
...
def wait_playable(client, timeout: float = 180.0) -> tuple[dict, dict]:
    ...
    while time.time() < deadline:
        try:
            ...
            last = {"error": str(error)[:160]}
        time.sleep(1.0)
    raise RuntimeError(f"PEERTUBE player did not become playable/injected: {last}")
...
def wait_media_job(db: str, timeout: float = 120.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(1.0)
    raise RuntimeError(f"no PEERTUBE media job appeared; jobs seen: {last}")
...
def hls_reference(source: str, root: Path) -> tuple[Path, dict]:
    ...
    if "#EXTM3U" not in manifest:
        raise RuntimeError("PEERTUBE source was not an HLS manifest")
    selected = source
    ...
    for line in variant_body.splitlines():
        ...
        if stripped.startswith("#EXT-X-MAP:"):
            ...
            if not uri:
                raise RuntimeError("PEERTUBE HLS initialization map was missing URI")
            ...
            if pending_range is not None:
                ...
                if offset is None:
                    if previous_url != url or previous_end is None:
                        raise RuntimeError("PEERTUBE HLS byte range omitted its offset without a prior range on the same resource")
                    start = previous_end
                else:
                    start = offset
    ...
    if not fragment_specs:
        raise RuntimeError("PEERTUBE HLS variant contained no finite fragments")
    ...
    track_path = root / "peertube-reference.track"
    ...
    with track_path.open("wb") as handle:
        for body in fragment_bodies:
            handle.write(body)
    (root / "peertube-reference.m3u8").write_text(variant_body, encoding="utf-8")
    reference = root / "peertube-reference.mp4"
    result = subprocess.run(
        ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(track_path), "-map", "0", "-c", "copy", str(reference)],
        capture_output=True, text=True, check=False,
    )
    if result.returncode != 0:
        raise RuntimeError(f"PEERTUBE reference ffmpeg failed: {result.stderr.strip()[-1000:]}")
    return reference, {
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-public-peertube-chromium-"))
    ...
    try:
        ...
        if consent:
            ...
            if consent_visible(client):
                ...
                print("PEERTUBE-CONSENT-FALLBACK: invoked the site's close handler after trusted close did not clear the banner", flush=True)
            deadline = time.time() + 10
            while consent_visible(client) and time.time() < deadline: time.sleep(0.5)
            if consent_visible(client): raise RuntimeError("PEERTUBE consent banner remained after close")
        state_before = wait_video_ready(client)
        if state_before.get("readyState", 0) >= 2 and not state_before.get("paused", True):
            pause = pause_point(client)
            if not pause:
                raise RuntimeError(f"PeerTube autoplayed but Pause control was not visible: {state_before}")
            click(client, pause)
            deadline = time.time() + 20
            while time.time() < deadline:
                if peertube_state(client).get("paused", True):
                    break
                time.sleep(0.5)
            if not peertube_state(client).get("paused", True):
                raise RuntimeError("PeerTube Pause gesture did not pause the video")
            print("PEERTUBE-PAUSE-CLICK: trusted CDP click issued to expose Play control", flush=True)
        point = wait_play_control(client)
        click(client, point); print("PEERTUBE-PLAY-CLICK: trusted CDP click issued", flush=True)
        state, button = wait_playable(client)
        print("PEERTUBE-PLAYER:", json.dumps(state, sort_keys=True), flush=True)
        print("PEERTUBE-BUTTON:", json.dumps(button, sort_keys=True), flush=True)
        button_diagnostic = evaluate_json(client, """(()=>{const item=document.querySelector('#dm-media-download-button');if(!item)return {error:'button disappeared'};const r=item.getBoundingClientRect();window.__dmPeerTubeButtonEvent=null;item.addEventListener('click',event=>{window.__dmPeerTubeButtonEvent={isTrusted:event.isTrusted,defaultPrevented:event.defaultPrevented,target:event.target?.id||event.target?.tagName||null};},{capture:true});const top=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return {outer:item.outerHTML,rect:{x:r.x,y:r.y,w:r.width,h:r.height},pointerEvents:getComputedStyle(item).pointerEvents,top:top?.outerHTML?.slice(0,500)||null};})()""")
        print("PEERTUBE-BUTTON-DIAGNOSTIC:", json.dumps(button_diagnostic, sort_keys=True), flush=True)
        click(client, button)
        time.sleep(0.5)
        print("PEERTUBE-BUTTON-EVENT:", json.dumps(evaluate_json(client, "JSON.stringify(window.__dmPeerTubeButtonEvent)"), sort_keys=True), flush=True)
        db = public.db_path(str(home)); created = wait_media_job(db)
        source = created.get("source") or ""
        if not source or source.startswith("blob:"): raise RuntimeError(f"PEERTUBE native job did not expose a replayable source: {redact_url(source)}")
        print("PEERTUBE-JOB:", json.dumps({"id":created.get("id"),"media":created.get("media"),"kind":created.get("kind"),"source":redact_url(source)}, sort_keys=True), flush=True)
        browser_reference = browser_source_reference(chrome_port, source)
        print("PEERTUBE-BROWSER-SOURCE-REFERENCE:", json.dumps({**browser_reference,"url":redact_url(source)}, sort_keys=True), flush=True)
        assert browser_reference.get("hls") is True and browser_reference.get("bytes", 0) > 0, browser_reference
        reference, hls_meta = hls_reference(source, root); print("PEERTUBE-HLS-REFERENCE:", json.dumps(hls_meta, sort_keys=True), flush=True)
        destination = managed_dir / "peertube-capture.mp4"; public.commit_via_cli(str(home), inspector_port, created["id"], destination.name, str(destination))
        ...
        print(f"PEERTUBE: PASS (output_bytes={len(output)}, output_sha256={output_hash}, browser_source_bytes={browser_reference['bytes']}, browser_source_sha256={browser_reference['sha256']}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("PUBLIC-PEERTUBE-CHROMIUM-PROBE: PASS", flush=True); return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_PUBLIC_PEERTUBE") != "1": shutil.rmtree(root, ignore_errors=True)


if __name__ == "__main__":
    try: raise SystemExit(main())
    except Exception as error:
        print(f"PUBLIC-PEERTUBE-CHROMIUM-PROBE: FAIL: {error}", flush=True); raise
