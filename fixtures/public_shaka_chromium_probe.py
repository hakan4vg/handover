#!/usr/bin/env python3
"""Real Chromium + Shaka public-player media initiation proof.

The official Shaka demo uses the documented HTML5 ``<source>`` form and a
...
VIDEO_PAGE = "https://shaka-project.github.io/shaka-player/demo/"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"Shaka capture created no native media job: {last}")


def activate_demo_play(client: adaptive.EventCDP) -> dict:
    deadline = time.time() + 30
    point = None
    while time.time() < deadline:
        raw = client.evaluate(
            "JSON.stringify((()=>{const all=(root)=>{const out=[];for(const x of root.querySelectorAll('*')){out.push(x);if(x.shadowRoot)out.push(...all(x.shadowRoot));}return out};const b=all(document).find(x=>x.textContent.trim()==='PLAY');"
            "if(!b)return null;const r=b.getBoundingClientRect();return {text:b.textContent.trim(),x:r.left+r.width/2,y:r.top+r.height/2,width:r.width,height:r.height}})())"
        )
        point = json.loads(raw) if isinstance(raw, str) else raw
        if point and point["width"] > 0 and point["height"] > 0:
            break
        time.sleep(0.2)
    if not point:
        diagnostic = client.evaluate("JSON.stringify({body:document.body?.innerText?.slice(0,1200)||'',buttons:Array.from(document.querySelectorAll('*')).filter(x=>x.textContent?.trim()==='PLAY').slice(0,10).map(x=>({tag:x.tagName,role:x.getAttribute('role'),class:x.className,html:x.outerHTML.slice(0,400)}))})")
        raise RuntimeError(f"Shaka demo PLAY button missing: {diagnostic}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    diagnostic = None
    try:
        diagnostic = client.evaluate("JSON.stringify({href:location.href,title:document.title,readyState:document.readyState,body:document.body?.innerText?.slice(0,800)||'',videos:document.querySelectorAll('video').length,videoHtml:Array.from(document.querySelectorAll('video')).map(v=>v.outerHTML.slice(0,1200)),sources:Array.from(document.querySelectorAll('video source')).map(s=>({attr:s.getAttribute('src'),prop:s.src,type:s.getAttribute('type')})),iframes:Array.from(document.querySelectorAll('iframe')).map(frame=>frame.src).slice(0,8)})")
    except Exception as error:
        diagnostic = repr(error)
    raise RuntimeError(f"Shaka player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"Shaka player surface missing: {raw}")
    client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
    client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"Shaka player did not instantiate after surface click: {raw}")
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if "storage.googleapis.com" not in host and "shaka-project.github.io" not in host:
            continue
        if not urlsplit(url).path.lower().endswith((".mp4", ".webm", ".ogv")):
            continue
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-shaka-chromium-"))
    home = root / "home"
    profile = root / "profile"
    downloads = profile / "Default" / "Downloads"
    managed = root / "Managed" / "shaka-blue-moon.mp4"
    ...
    try:
        ...
        play_point = activate_demo_play(client)
        print("SHAKA-DEMO-PLAY:", json.dumps(play_point, sort_keys=True), flush=True)
        player = wait_player(client)
        print("SHAKA-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("SHAKA-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("SHAKA-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        print("SHAKA-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("SHAKA-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        public.commit_via_cli(str(home), inspector_port, job["id"], "shaka-blue-moon.mp4", str(managed))
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            ...
            print("BROWSER-SHAKA-REFERENCE:", json.dumps({"url": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("SHAKA-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"SHAKA-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("SHAKA-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_SHAKA") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"SHAKA-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
