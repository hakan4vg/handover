#!/usr/bin/env python3
"""Real Chromium/native proof: generic media capture on a production podcast page.

Transistor's real public episode share page (production host, not a library
demo) ships the episode's mp3 in the top document: a native <audio> with
src=https://media.transistor.fm/e70577fd/6bc50f3d.mp3?src=player and
preload="none". A trusted click on the real player control
(button.play-button[aria-label="Play/Pause"]) starts playback; the extension's
generic media button must appear, and the resident must capture the exact mp3
(byte/hash-identical to an independent fetch of the same source).
"""
...
PAGE = "https://share.transistor.fm/s/e70577fd"
EXPECTED_SOURCE_HOST = "media.transistor.fm"
BIN = public.BIN
...
def wait_page(client, timeout: float = 60.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            last = evaluate_json(
                client,
                "{href:location.href,state:document.readyState,audio:[...document.querySelectorAll('audio')].map(a=>a.currentSrc||a.src||''),play:!!document.querySelector('button.play-button[aria-label=\\\"Play/Pause\\\"]')}",
            )
            if (
                last.get("state") == "complete"
                and last.get("href", "").startswith("https://share.transistor.fm/")
                and any(EXPECTED_SOURCE_HOST in src for src in last.get("audio", []))
                and last.get("play")
            ):
                return last
        except Exception:
            pass
        time.sleep(0.5)
    raise RuntimeError(f"Transistor page did not become ready: {last}")


def play_control(client) -> dict:
    result = evaluate_json(
        client,
        "(()=>{const b=document.querySelector('button.play-button[aria-label=\\\"Play/Pause\\\"]');if(!b)return {error:'play control missing'};"
...
def wait_playing(client, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        last = evaluate_json(
            client,
            "(()=>{const media=[...document.querySelectorAll('video,audio')].find(el=>{const src=el.currentSrc||el.src||'';return /^https?:/.test(src)&&!el.paused&&el.readyState>=1&&!el.ended;});"
        ...
        if last.get("found") and last.get("button") and last.get("buttonRect"):
            return last
        time.sleep(0.5)
    raise RuntimeError(f"Transistor player did not become playable/injected: {last}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-transistor-"))
    home = root / "home"
    ...
    try:
        ...
        print("TRANSISTOR-PAGE:", json.dumps(page, sort_keys=True), flush=True)
        ...
        print("TRANSISTOR-PLAY-CONTROL:", json.dumps(control, sort_keys=True), flush=True)
        click(client, control)
        playing = wait_playing(client)
        print("TRANSISTOR-PLAYING:", json.dumps(playing, sort_keys=True), flush=True)
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.5)
        if not job:
            raise RuntimeError(f"no native media job for Transistor audio: {public.jobs(db)}")
        print("TRANSISTOR-NATIVE-JOB:", json.dumps({"id": job["id"], "source": job["source"], "media": job.get("media"), "state": job.get("state"), "name": job.get("name")}, sort_keys=True), flush=True)
        media_name = "transistor-episode.mp3"
        managed = managed_dir / media_name
        public.commit_via_cli(str(home), inspector_port, job["id"], media_name, str(managed))
        reference_dir = root / "reference"
        reference_dir.mkdir(parents=True, exist_ok=True)
        reference_path = reference_dir / "source.mp3"
        reference_path.write_bytes(fetch_bytes(job["source"]))
        reference = {"size": reference_path.stat().st_size, "sha256": sha256(reference_path)}
        print("TRANSISTOR-REFERENCE:", json.dumps(reference, sort_keys=True), flush=True)
        ...
        print(f"TRANSISTOR-PODCAST: PASS (page={PAGE}, source={job['source']}, output_bytes={native_size}, sha256={native_hash}, jobs={len(public.jobs(db))}, browser_downloads={browser_files})", flush=True)
        print("TRANSISTOR-PODCAST-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if xvfb.poll() is None:
            ...
            try:
                ...
                xvfb.kill()
                xvfb.wait(timeout=5)
        if os.environ.get("DM_KEEP_TRANSISTOR") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"TRANSISTOR-PODCAST-PROBE: FAIL: {error}", flush=True)
        raise