#!/usr/bin/env python3
"""Real Chromium + Vime progressive MP4 proof.

Vime's official demo uses a custom accessible controller surface around
an MP4 source. This probe clicks the first real Vime Play control with
...
TARGET = "video"
SOURCE_HOST = "files.vidstack.io"
SOURCE_PATH = "/agent-327/720p.mp4"
...
def player_state(client: adaptive.EventCDP) -> dict:
    raw = client.evaluate(
        ...
        "const roots=[];const seen=new Set();const walk=root=>{if(!root||seen.has(root))return;seen.add(root);roots.push(root);for(const e of root.querySelectorAll?.('*')||[])if(e.shadowRoot)walk(e.shadowRoot)};walk(document);const play=roots.flatMap(root=>[...root.querySelectorAll?.('vm-playback-control,button,[role=\\\"button\\\"]')||[]]).find(item=>item.tagName==='VM-PLAYBACK-CONTROL'||item.getAttribute('aria-label')==='Playback');const pr=play?.getBoundingClientRect();"
...
def wait_ready(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"Vime media did not become ready: {last}")
...
def wait_playing(client: adaptive.EventCDP, timeout: float = 60.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"Vime media did not become playing/injected: {last}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-vime-chromium-"))
    home = root / "home"
    profile = root / "profile"
    downloads = profile / "Default" / "Downloads"
    managed = root / "Managed" / "vime-deadline.mp4"
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            raise RuntimeError(f"Vime page did not load: {loaded}")

        player = wait_ready(client)
        print("VIME-PLAYER-READY:", json.dumps(player, sort_keys=True), flush=True)
        click(client, player["playPoint"])
        print("VIME-PLAY-CLICK: trusted CDP input issued", flush=True)
        player = wait_playing(client)
        print("VIME-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        source = player["source"]
        source_parts = urlsplit(source)
        if source_parts.netloc.lower() != SOURCE_HOST or source_parts.path != SOURCE_PATH:
            raise RuntimeError(f"Vime selected unexpected source: {source}")

        click(client, player["buttonPoint"])
        print("VIME-DOWNLOAD-CLICK: trusted CDP input issued", flush=True)
        db = public.db_path(str(home))
        job = public.wait_job(db, source, timeout=90)
        print("VIME-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "provisional": job.get("provisional"), "name": job.get("name")}, sort_keys=True), flush=True)
        if job.get("media") is not True or job.get("source") != source:
            raise RuntimeError(f"wrong Vime native job: {job}")
        if len(public.jobs(db)) != 1:
            raise RuntimeError(f"unexpected extra Vime job: {public.jobs(db)}")

        reference = independent_reference(source)
        print("VIME-INDEPENDENT-REFERENCE:", json.dumps({"url": redacted_url(source), "bytes": len(reference), "sha256": sha256_bytes(reference)}, sort_keys=True), flush=True)
        public.commit_via_cli(str(home), inspector_port, job["id"], "vime-deadline.mp4", str(managed))
        ...
        print("VIME-NATIVE-RESULT:", json.dumps({"state": done.get("state"), "provisional": done.get("provisional"), "bytes": len(output), "sha256": output_hash}, sort_keys=True), flush=True)
        assert len(public.jobs(db)) == 1, public.jobs(db)
        assert not browser_files, browser_files
        print(f"VIME: PASS (page={PAGE}, source={redacted_url(source)}, output_bytes={len(output)}, output_sha256={output_hash}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("VIME-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if xvfb.poll() is None:
            ...
            try:
                ...
                xvfb.kill()
                xvfb.wait(timeout=5)
        if os.environ.get("DM_KEEP_VIME") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"VIME-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
