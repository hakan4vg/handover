#!/usr/bin/env python3
"""Real Chromium/native proof for Vime's official HLS custom-element demo.

Vime's provider selector mounts a real shadow-root custom player whose native
video is backed by a finite HLS master playlist. The probe reaches the actual
vm-playback-control with trusted input, clicks the injected Download Manager
button, commits through the resident CLI, and compares the managed output with
an independent finite-HLS reconstruction.
"""
...
def wait_page(client, timeout: float = 90.0) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"VIME page did not load: {last}")
...
def wait_point(client, getter, label: str, timeout: float = 120.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        time.sleep(1.0)
    raise RuntimeError(f"VIME {label} did not mount: {last}")


def select_hls_provider(client) -> dict:
    result = evaluate_json(client, """(()=>{const select=[...document.querySelectorAll('select')].find(item=>[...item.options].some(option=>option.value==='hls'));if(!select)return {ok:false,error:'Vime provider select missing'};const setter=Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value')?.set;if(!setter)return {ok:false,error:'HTMLSelectElement value setter missing'};setter.call(select,'hls');select.dispatchEvent(new Event('change',{bubbles:true}));const source=document.querySelector('vm-hls source');return {ok:select.value==='hls',value:select.value,vmHls:!!document.querySelector('vm-hls'),source:source?.getAttribute('data-src')||source?.getAttribute('src')||''};})()""")
    if not result.get("ok"):
        raise RuntimeError(f"Vime HLS provider selection failed: {result}")
    return result


def wait_hls_provider(client, timeout: float = 60.0) -> dict:
    deadline = time.time() + timeout
    last = None
    while time.time() < deadline:
        try:
            last = evaluate_json(client, """(()=>{const source=document.querySelector('vm-hls source');const video=document.querySelector('video');const player=document.querySelector('vm-player');return {vmHls:!!document.querySelector('vm-hls'),playerReady:!!player?.hasAttribute('ready'),source:source?.getAttribute('data-src')||source?.getAttribute('src')||'',rawSrc:video?.currentSrc||video?.src||'',readyState:video?.readyState||0,duration:Number.isFinite(video?.duration)?video.duration:0};})()""")
            if last.get("vmHls") and last.get("playerReady") and last.get("readyState", 0) >= 2 and last.get("duration", 0) > 0 and "/hls/" in last.get("source", "") and last.get("source", "").split("?", 1)[0].endswith(".m3u8"):
                return last
        except Exception as error:
            last = {"error": str(error)[:160]}
        time.sleep(0.5)
    raise RuntimeError(f"Vime HLS provider did not mount: {last}")


def play_point(client) -> dict | None:
    return evaluate_json(client, """(()=>{const roots=[];const seen=new Set();const walk=root=>{if(!root||seen.has(root))return;seen.add(root);roots.push(root);for(const e of root.querySelectorAll?.('*')||[])if(e.shadowRoot)walk(e.shadowRoot)};walk(document);const item=roots.flatMap(root=>[...root.querySelectorAll?.('vm-playback-control,button,[role=\\\"button\\\"]')||[]]).find(e=>e.tagName==='VM-PLAYBACK-CONTROL'||e.getAttribute('aria-label')==='Playback');if(!item)return null;const r=item.getBoundingClientRect();return r.width>0&&r.height>0?{x:r.left+r.width/2,y:r.top+r.height/2,text:item.getAttribute('aria-label')||item.tagName}:null})()""")


def wait_play_control(client, timeout: float = 120.0) -> dict:
    return wait_point(client, play_point, "Vime Playback control", timeout)
...
def vime_state(client) -> dict:
    return evaluate_json(client, """(()=>{const v=document.querySelector('video');const r=v?.getBoundingClientRect();const raw=v?.currentSrc||v?.src||'';const source=raw.startsWith('blob:')?(v?.querySelector('source[data-src]')?.getAttribute('data-src')||v?.querySelector('source[src]')?.getAttribute('src')||raw):raw;return {src:source,rawSrc:raw,readyState:v?.readyState||0,paused:v?.paused??true,currentTime:v?.currentTime||0,duration:Number.isFinite(v?.duration)?v.duration:0,networkState:v?.networkState||0,error:v?.error?{code:v.error.code,message:v.error.message}:null,rect:r?{x:r.x,y:r.y,w:r.width,h:r.height}:null}})()""")
...
def wait_playable(client, timeout: float = 180.0) -> tuple[dict, dict]:
    ...
    while time.time() < deadline:
        try:
            ...
            last = {"error": str(error)[:160]}
        time.sleep(1.0)
    raise RuntimeError(f"VIME player did not become playable/injected: {last}")
...
def wait_media_job(db: str, timeout: float = 120.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(1.0)
    raise RuntimeError(f"no VIME media job appeared; jobs seen: {last}")
...
def hls_reference(source: str, root: Path) -> tuple[Path, dict]:
    ...
    if "#EXTM3U" not in manifest:
        raise RuntimeError("VIME source was not an HLS manifest")
    selected = source
    ...
    for line in variant_body.splitlines():
        ...
        if stripped.startswith("#EXT-X-MAP:"):
            ...
            if not uri:
                raise RuntimeError("VIME HLS initialization map was missing URI")
            ...
            if pending_range is not None:
                ...
                if offset is None:
                    if previous_url != url or previous_end is None:
                        raise RuntimeError("VIME HLS byte range omitted its offset without a prior range on the same resource")
                    start = previous_end
                else:
                    start = offset
    ...
    if not fragment_specs:
        raise RuntimeError("VIME HLS variant contained no finite fragments")
    ...
    track_path = root / "vime-reference.track"
    ...
    with track_path.open("wb") as handle:
        for body in fragment_bodies:
            handle.write(body)
    (root / "vime-reference.m3u8").write_text(variant_body, encoding="utf-8")
    reference = root / "vime-reference.mp4"
    result = subprocess.run(
        ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(track_path), "-map", "0", "-c", "copy", str(reference)],
        capture_output=True, text=True, check=False,
    )
    if result.returncode != 0:
        raise RuntimeError(f"VIME reference ffmpeg failed: {result.stderr.strip()[-1000:]}")
    return reference, {
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-public-vime-chromium-", dir="/var/tmp"))
    ...
    try:
        ...
        selection = select_hls_provider(client)
        provider = wait_hls_provider(client)
        print("VIME-HLS-SELECTION:", json.dumps({**selection, "source": redact_url(selection.get("source", ""))}, sort_keys=True), flush=True)
        print("VIME-HLS-PROVIDER:", json.dumps({**provider, "source": redact_url(provider.get("source", "")), "rawSrc": redact_url(provider.get("rawSrc", ""))}, sort_keys=True), flush=True)
        ...
        print("VIME-PLAY-CONTROL:", json.dumps(point, sort_keys=True), flush=True)
        click(client, point)
        print("VIME-PLAY-CLICK: trusted CDP click issued", flush=True)
        state, button = wait_playable(client)
        print("VIME-PLAYER:", json.dumps(state, sort_keys=True), flush=True)
        print("VIME-BUTTON:", json.dumps(button, sort_keys=True), flush=True)
        button_diagnostic = evaluate_json(client, """(()=>{const item=document.querySelector('#dm-media-download-button');if(!item)return {error:'button disappeared'};const r=item.getBoundingClientRect();window.__dmVimeButtonEvent=null;item.addEventListener('click',event=>{window.__dmVimeButtonEvent={isTrusted:event.isTrusted,defaultPrevented:event.defaultPrevented,target:event.target?.id||event.target?.tagName||null};},{capture:true});const top=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return {outer:item.outerHTML,rect:{x:r.x,y:r.y,w:r.width,h:r.height},pointerEvents:getComputedStyle(item).pointerEvents,top:top?.outerHTML?.slice(0,500)||null};})()""")
        print("VIME-BUTTON-DIAGNOSTIC:", json.dumps(button_diagnostic, sort_keys=True), flush=True)
        click(client, button)
        time.sleep(0.5)
        print("VIME-BUTTON-EVENT:", json.dumps(evaluate_json(client, "JSON.stringify(window.__dmVimeButtonEvent)"), sort_keys=True), flush=True)
        db = public.db_path(str(home)); created = wait_media_job(db)
        source = created.get("source") or ""
        if not source or source.startswith("blob:"): raise RuntimeError(f"VIME native job did not expose a replayable source: {redact_url(source)}")
        print("VIME-JOB:", json.dumps({"id":created.get("id"),"media":created.get("media"),"kind":created.get("kind"),"source":redact_url(source)}, sort_keys=True), flush=True)
        browser_reference = browser_source_reference(chrome_port, source)
        print("VIME-BROWSER-SOURCE-REFERENCE:", json.dumps({**browser_reference,"url":redact_url(source)}, sort_keys=True), flush=True)
        assert browser_reference.get("hls") is True and browser_reference.get("bytes", 0) > 0, browser_reference
        reference, hls_meta = hls_reference(source, root); print("VIME-HLS-REFERENCE:", json.dumps(hls_meta, sort_keys=True), flush=True)
        destination = managed_dir / "vime-capture.mp4"; public.commit_via_cli(str(home), inspector_port, created["id"], destination.name, str(destination))
        ...
        print("VIME-NATIVE-RESULT:", json.dumps({"state": completed.get("state"), "provisional": completed.get("provisional"), "bytes": len(output), "sha256": output_hash}, sort_keys=True), flush=True)
        print(f"VIME: PASS (output_bytes={len(output)}, output_sha256={output_hash}, browser_source_bytes={browser_reference['bytes']}, browser_source_sha256={browser_reference['sha256']}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("PUBLIC-VIME-CHROMIUM-PROBE: PASS", flush=True); return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_PUBLIC_VIME") != "1": shutil.rmtree(root, ignore_errors=True)


if __name__ == "__main__":
    try: raise SystemExit(main())
    except Exception as error:
        print(f"PUBLIC-VIME-CHROMIUM-PROBE: FAIL: {error}", flush=True); raise
