#!/usr/bin/env python3
"""Real Chromium + VisionPlayer progressive AV1 MP4 proof.

VisionPlayer's official trailer uses a custom overlay/controller surface around
an AV1 MP4 source. This probe clicks the real VisionPlayer Play control with
...
PAGE = "https://visionplayer.io/"
TARGET = "#vip-engine-video"
SOURCE_HOST = "visionplayer.io"
SOURCE_PATH = "/media/trailer/streams/visionplayer-trailer.en.720.av1.mp4"
...
def player_state(client: adaptive.EventCDP) -> dict:
    raw = client.evaluate(
        ...
        "const play=document.querySelector('.play[aria-label=\\\"Play\\\"]');const pr=play?.getBoundingClientRect();"
...
def wait_ready(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"VisionPlayer media did not become ready: {last}")
...
def wait_playing(client: adaptive.EventCDP, timeout: float = 60.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"VisionPlayer media did not become playing/injected: {last}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-visionplayer-chromium-"))
    home = root / "home"
    profile = root / "profile"
    downloads = profile / "Default" / "Downloads"
    managed = root / "Managed" / "visionplayer-trailer.mp4"
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            raise RuntimeError(f"VisionPlayer page did not load: {loaded}")

        player = wait_ready(client)
        print("VISIONPLAYER-PLAYER-READY:", json.dumps(player, sort_keys=True), flush=True)
        click(client, player["playPoint"])
        print("VISIONPLAYER-PLAY-CLICK: trusted CDP input issued", flush=True)
        player = wait_playing(client)
        print("VISIONPLAYER-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        source = player["source"]
        source_parts = urlsplit(source)
        if source_parts.netloc.lower() != SOURCE_HOST or source_parts.path != SOURCE_PATH:
            raise RuntimeError(f"VisionPlayer selected unexpected source: {source}")

        click(client, player["buttonPoint"])
        print("VISIONPLAYER-DOWNLOAD-CLICK: trusted CDP input issued", flush=True)
        db = public.db_path(str(home))
        job = public.wait_job(db, source, timeout=90)
        print("VISIONPLAYER-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "provisional": job.get("provisional"), "name": job.get("name")}, sort_keys=True), flush=True)
        if job.get("media") is not True or job.get("source") != source:
            raise RuntimeError(f"wrong VisionPlayer native job: {job}")
        if len(public.jobs(db)) != 1:
            raise RuntimeError(f"unexpected extra VisionPlayer job: {public.jobs(db)}")

        reference = independent_reference(source)
        print("VISIONPLAYER-INDEPENDENT-REFERENCE:", json.dumps({"url": redacted_url(source), "bytes": len(reference), "sha256": sha256_bytes(reference)}, sort_keys=True), flush=True)
        public.commit_via_cli(str(home), inspector_port, job["id"], "visionplayer-trailer.mp4", str(managed))
        ...
        print(f"VISIONPLAYER: PASS (page={PAGE}, source={redacted_url(source)}, output_bytes={len(output)}, output_sha256={output_hash}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("VISIONPLAYER-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if xvfb.poll() is None:
            ...
            try:
                ...
                xvfb.kill()
                xvfb.wait(timeout=5)
        if os.environ.get("DM_KEEP_VISION") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"VISIONPLAYER-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
