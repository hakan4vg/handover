#!/usr/bin/env python3
"""Real Chromium + W3Schools iframe audio public-player media initiation proof.

The official W3Schools iframe audio demo uses the documented HTML5 ``<source>`` form and a
...
VIDEO_PAGE = "https://www.w3schools.com/html/tryit.asp?filename=tryhtml5_audio_all"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, source: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"W3Schools iframe audio capture created no native media job for {source!r}: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            last_raw = repr(raw)
            last = json.loads(raw)
            players = [item for item in last.get("players", []) if item.get("source", "").startswith(("http://", "https://")) and item.get("source", "").find("w3schools.com") >= 0 and item.get("source", "").lower().endswith((".ogg", ".mp3", ".wav")) and item.get("visible") and item.get("button") and item.get("clickPoint")]
            if players:
    ...
    raise RuntimeError(f"W3Schools iframe audio player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"W3Schools iframe audio player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"W3Schools iframe audio player did not instantiate after surface click: {raw}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-w3-iframe-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        assert player["frameUrl"].startswith("https://www.w3schools.com/"), player
        ...
        media_name = "w3-iframe-audio" + suffix
        ...
        if os.environ.get("DM_KEEP_W3_IFRAME_AUDIO") != "1":
