#!/usr/bin/env python3
"""Real Chromium + W3C recycled-player/source-switch media proof.

The official W3C HTML5 Video Events demonstration reuses one top-level
``<video>`` element. Its trusted ``Test movie`` control changes the child
sources to an alternate test movie and calls ``load()``; Chromium selects the
existing MP4 child while retaining the WebM fallback. The probe uses a fresh
Chromium profile, the real unpacked extension/native host, a resident real
binary, and browser CDP evidence; it clicks the product's player-bound
Download button after the source switch, commits through the resident CLI,
and compares the native result with browser-side bytes.
"""
...
VIDEO_PAGE = "https://www.w3.org/2010/05/video/mediaevents.html"
PAGE = VIDEO_PAGE
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"W3C source-switch capture created no native media job: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    raise RuntimeError(f"W3C source-switch player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"W3C source-switch player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"W3C source-switch player did not instantiate after surface click: {raw}")
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-w3c-source-switch-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        switch_point = json.loads(client.evaluate(
            "JSON.stringify((()=>{"
            "const b=Array.from(document.querySelectorAll('#buttons button')).find(item=>item.textContent.trim()==='Test movie');"
            "if(!b)throw Error('W3C Test movie control missing');"
            "const r=b.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2,text:b.textContent.trim()};})())"
        ))
        print("W3C-SOURCE-SWITCH-CONTROL:", json.dumps(switch_point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": switch_point["x"], "y": switch_point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": switch_point["x"], "y": switch_point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        player = wait_player(client)
        assert player["src"].lower().endswith("movie_300.mp4"), player
        print("W3C-SWITCHED-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("W3C-SWITCHED-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("W3C-SWITCHED-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".media"
        media_name = "w3c-movie-300" + suffix
        managed = managed_dir / media_name
        print("W3C-SWITCHED-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("W3C-SWITCHED-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            reference_url = job["source"]
            if reference_url.startswith("http://media.w3.org/"):
                reference_url = "https://" + reference_url[len("http://"):]
            client.call("Page.navigate", {"url": "https://media.w3.org/"})
            time.sleep(2)
            browser_reference_result = browser_reference(client, reference_url)
            print("BROWSER-W3C-SWITCH-REFERENCE:", json.dumps({"url": redacted_url(reference_url), "job_source": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("W3C-SWITCHED-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"W3C-SOURCE-SWITCH-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("W3C-SOURCE-SWITCH-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
        ...
        if os.environ.get("DM_KEEP_W3C_SWITCH") != "1":
...
if __name__ == "__main__":
    try:
        ...
        print(f"W3C-SOURCE-SWITCH-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
