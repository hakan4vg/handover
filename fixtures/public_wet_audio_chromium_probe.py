#!/usr/bin/env python3
"""Real Chromium + WET audio public-player media initiation proof.

The official Web Experience Toolkit audio demo uses a documented HTML5 ``<audio>`` element with MP3 and OGG ``<source>`` children. It uses a fresh Chromium profile, the real unpacked
...
VIDEO_PAGE = "https://wet-boew.github.io/wet-boew/demos/multimedia/multimedia-audio-en.html"
PAGE = VIDEO_PAGE
...
def browser_reference(client: adaptive.EventCDP, url: str) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            href = last.get("href") if isinstance(last, dict) else None
            if isinstance(href, str) and href.startswith(("http://", "https://")) and last.get("state") == "complete":
                resolved_url = href
                break
        except Exception:
            pass
        time.sleep(0.5)
    if not resolved_url:
        raise RuntimeError(f"browser reference navigation failed for {redacted_url(url)}: {last}")
    raw = client.evaluate(
        "(async()=>{"
        "const u=location.href;const r=await fetch(u,{cache:'no-store'});"
        "if(!r.ok)return JSON.stringify({error:'HTTP '+r.status});"
        "const b=new Uint8Array(await r.arrayBuffer());"
        "const d=new Uint8Array(await crypto.subtle.digest('SHA-256',b));"
        "return JSON.stringify({resolvedUrl:u,size:b.length,hash:Array.from(d).map(x=>x.toString(16).padStart(2,'0')).join('')});"
        "})()"
    )
    result = json.loads(raw)
    if result.get("error"):
        raise RuntimeError(f"browser reference fetch failed for {redacted_url(resolved_url)}: {result['error']}")
    return result
...
def wait_media_job(db: str, timeout: float = 90.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if last:
            return last[-1]
        time.sleep(0.2)
    raise RuntimeError(f"WET audio capture created no native media job: {last}")
...
def wait_player(client: adaptive.EventCDP, timeout: float = 90.0) -> dict:
    ...
    raise RuntimeError(f"WET audio player did not become playable/injected: last={last}; raw={last_raw}; error={last_error}; diagnostic={diagnostic}")
...
def activate_player(client: adaptive.EventCDP) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    if not point:
        raise RuntimeError(f"WET audio player surface missing: {raw}")
    ...
    while time.time() < deadline:
        ...
        if state.get("source") and not state.get("disabled"):
            return
        time.sleep(0.5)
    raise RuntimeError(f"WET audio player did not instantiate after surface click: {raw}")
...
def collect_media_events(client: adaptive.EventCDP) -> list[dict]:
    ...
    for event in client.take_events():
        ...
        if "archive.org" not in host and "wet-boew.github.io" not in host:
            continue
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="dm-wet-audio-chromium-"))
    home = root / "home"
    ...
    try:
        ...
        print("WET-AUDIO-PLAYER:", json.dumps(player, sort_keys=True), flush=True)
        initial_traffic = wait_for_traffic(client)
        print("WET-AUDIO-TRAFFIC:", json.dumps(initial_traffic[:12], sort_keys=True), flush=True)
        point = button_point(client)
        print("WET-AUDIO-BUTTON:", json.dumps(point, sort_keys=True), flush=True)
        client.call("Input.dispatchMouseEvent", {"type": "mousePressed", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        client.call("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": point["x"], "y": point["y"], "button": "left", "clickCount": 1, "modifiers": 0})
        db = public.db_path(str(home))
        job = wait_media_job(db)
        suffix = Path(urlsplit(job["source"]).path).suffix or ".audio"
        media_name = "wet-boew-audio" + suffix
        managed = managed_dir / media_name
        print("WET-AUDIO-NATIVE-JOB:", json.dumps({"id": job["id"], "source": redacted_url(job["source"]), "media": job.get("media"), "state": job.get("state"), "name": job.get("name"), "selected_segments": [redacted_url(url) for url in job.get("selected_segments", [])]}, sort_keys=True), flush=True)
        traffic = initial_traffic + collect_media_events(client)
        print("WET-AUDIO-SELECTED-TRAFFIC:", json.dumps(traffic[:24], sort_keys=True), flush=True)
        ...
        if job.get("source", "").startswith(("http://", "https://")):
            ...
            print("BROWSER-WET-AUDIO-REFERENCE:", json.dumps({"url": redacted_url(job["source"]), **browser_reference_result}, sort_keys=True), flush=True)
        ...
        print("WET-AUDIO-NATIVE-RESULT:", json.dumps({"state": done["state"], "provisional": done.get("provisional"), "bytes": native_size, "sha256": native_hash, "browser_downloads": browser_files}, sort_keys=True), flush=True)
        print(f"WET-AUDIO-CHROMIUM: PASS (page={PAGE}, source={redacted_url(job['source'])}, output_bytes={native_size}, output_sha256={native_hash}, traffic={len(traffic)}, jobs=1, browser_downloads={browser_files})", flush=True)
        print("WET-AUDIO-CHROMIUM-PROBE: PASS", flush=True)
        return 0
    except Exception:
...
if __name__ == "__main__":
    try:
        ...
        print(f"WET-AUDIO-CHROMIUM-PROBE: FAIL: {error}", flush=True)
        raise
