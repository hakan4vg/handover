...
function Manager({ adapter, snapshot }: { adapter: DownloadAdapter; snapshot: AppSnapshot }) {
  ...
  const run = (promise: Promise<void>, success?: string) => {
    promise.then(() => success && setNotice(success)).catch((reason: unknown) => setNotice(reason instanceof Error ? reason.message : 'Action failed'));
  };
  ...
  const handleManualSubmit = async (source: string, name: string, maxConnections: number) => {
    try {
      ...
      setNotice(reason instanceof Error ? reason.message : 'Could not start acquisition');
    }
  };
...
function EmptyState({ filter, onAdd }: { filter: FilterKey; onAdd: () => void }) {
  return <div className="empty-state"><div className="empty-icon"><Icon name={filter === 'failed' ? 'check' : 'download'} size={24} /></div><strong>No {filter} downloads</strong><span>Downloads matching this view will appear here.</span>{filter === 'all' && <button className="button primary" onClick={onAdd}><Icon name="add" size={16} /> Add URL</button>}</div>;
}
...
function JobContextMenu({ job, adapter, onClose, onNotice }: { job: DownloadJob; adapter: DownloadAdapter; onClose: () => void; onNotice: (message: string) => void }) {
  const run = (action: Promise<void>, message?: string) => action.then(() => { if (message) onNotice(message); onClose(); }).catch((reason: unknown) => onNotice(reason instanceof Error ? reason.message : 'Action failed'));
...
function GeneralSettings({ settings, update }: { settings: AppSettings; update: (patch: Partial<AppSettings>) => void }) {
  return <><SettingsHeading title="General" /><SettingCard><SettingToggle icon="power" title="Start download service at sign-in" description="Run the download service in the background when Windows starts." checked={settings.startAtSignIn} onChange={(startAtSignIn) => update({ startAtSignIn })} /><SettingToggle icon="window" title="Start manager UI at sign-in" description="Launch the manager interface automatically when Windows starts." checked={settings.showManagerAtSignIn} onChange={(showManagerAtSignIn) => update({ showManagerAtSignIn })} /><div className="setting-line"><Icon name="power" size={20} /><div className="setting-copy"><strong>Close button behavior</strong><span>Choose what happens when the main window close button is clicked.</span></div><Select value={settings.closeBehavior} onChange={(value) => update({ closeBehavior: value as AppSettings['closeBehavior'] })} options={[['tray', 'Minimize to tray'], ['exit', 'Exit application']]} /></div></SettingCard></>;
}

function DownloadSettings({ settings, update }: { settings: AppSettings; update: (patch: Partial<AppSettings>) => void }) {
  return <><SettingsHeading title="Downloads" /><SettingCard><FieldHeading title="Default download folder" description="All downloads will be saved to this folder." /><PathField value={settings.defaultFolder} onChange={(defaultFolder) => update({ defaultFolder })} /><FieldHeading title="Temporary / cache folder" description="Temporary files and parts will be stored here." /><PathField value={settings.tempFolder} onChange={(tempFolder) => update({ tempFolder })} /></SettingCard><SettingCard><FieldHeading title="File name collisions" description="Choose how to handle an existing file with the same name." /><Select value={settings.collisionBehavior} onChange={(collisionBehavior) => update({ collisionBehavior: collisionBehavior as AppSettings['collisionBehavior'] })} options={[['rename', 'Create a numbered copy'], ['replace', 'Replace existing file']]} /></SettingCard></>;
}
...
function NetworkSettings({ settings, update }: { settings: AppSettings; update: (patch: Partial<AppSettings>) => void }) {
  return <><SettingsHeading title="Network" /><div className="network-grid"><SettingCard><FieldHeading title="Global bandwidth limit (aggregate)" description="Applies to all active downloads combined." /><div className="radio-row"><Radio checked={settings.bandwidthLimit === null} onClick={() => update({ bandwidthLimit: null })} label="Unlimited" /><Radio checked={settings.bandwidthLimit !== null} onClick={() => update({ bandwidthLimit: 50 })} label="Limited to:" /><input className="number-input" type="number" min="1" value={settings.bandwidthLimit ?? 50} onChange={(event) => update({ bandwidthLimit: Number(event.target.value) || 1 })} /><Select value={settings.bandwidthUnit} onChange={(bandwidthUnit) => update({ bandwidthUnit: bandwidthUnit as AppSettings['bandwidthUnit'] })} options={[['KB/s', 'KB/s'], ['MB/s', 'MB/s'], ['GB/s', 'GB/s']]} /></div><FieldHeading title="Default maximum connections per download" description="Maximum number of connections used for each download." /><input className="number-input large" type="number" min="1" max="32" value={settings.maxConnections} onChange={(event) => update({ maxConnections: Math.max(1, Math.min(32, Number(event.target.value) || 1)) })} /><SettingToggle title="Per-download override allowed" description="Allow custom limits and connections per download." checked={settings.perDownloadOverrides} onChange={(perDownloadOverrides) => update({ perDownloadOverrides })} /><SettingToggle title="Retry failed downloads automatically" description="Retry downloads that fail due to network issues." checked={settings.retryAutomatically} onChange={(retryAutomatically) => update({ retryAutomatically })} /><FieldHeading title="Max retry attempts" description="Number of times to retry before giving up." /><input className="number-input large" type="number" min="0" max="20" value={settings.maxRetries} onChange={(event) => update({ maxRetries: Math.max(0, Math.min(20, Number(event.target.value) || 0)) })} /></SettingCard><NetworkLiveStats /></div></>;
}
...
function AppearanceSettings({ settings, update }: { settings: AppSettings; update: (patch: Partial<AppSettings>) => void }) {
  ...
  return <><SettingsHeading title="Appearance" /><SettingCard><FieldHeading title="Theme" /><div className="theme-options"><ThemeOption value="system" label="Follow system" icon="monitor" current={settings.theme} onClick={() => update({ theme: 'system' })} /><ThemeOption value="light" label="Light" icon="power" current={settings.theme} onClick={() => update({ theme: 'light' })} /><ThemeOption value="dark" label="Dark" icon="pending" current={settings.theme} onClick={() => update({ theme: 'dark' })} /></div></SettingCard><SettingCard><FieldHeading title="Accent color" /><div className="accent-options">{accents.map((accent) => <button key={accent} aria-label={`Use ${accent} accent`} className={`accent-swatch ${settings.accent === accent ? 'selected' : ''}`} style={{ '--swatch': accent } as CSSProperties} onClick={() => update({ accent })}><span /></button>)}</div></SettingCard><SettingCard><FieldHeading title="App density" description="Choose how compact the interface should be." /><Select value={settings.density} onChange={(density) => update({ density: density as AppSettings['density'] })} options={[['comfortable', 'Comfortable'], ['compact', 'Compact']]} /></SettingCard></>;
}
...
function ThemeOption({ value, label, icon, current, onClick }: { value: string; label: string; icon: IconName; current: string; onClick: () => void }) {
  return <button className={`theme-option ${current === value ? 'selected' : ''} theme-${value}`} onClick={onClick}><span className="theme-preview"><Icon name={icon} size={24} /></span><span>{label}</span></button>;
}
...
function PathField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="path-field"><input aria-label="Folder path" value={value} onChange={(event) => onChange(event.target.value)} /></div>;
}

function Select({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: Array<[string, string]> }) {
  return <label className="select-wrap"><select value={value} onChange={(event) => onChange(event.target.value)}>{options.map(([optionValue, label]) => <option key={optionValue} value={optionValue}>{label}</option>)}</select><Icon name="chevron-down" size={14} /></label>;
}
...
function AddDownloadWindow({ settings, job, onCreate, onCommit, onCancel, onClose }: { adapter?: DownloadAdapter; settings: AppSettings; job?: DownloadJob; onCreate?: (source: string, name: string, maxConnections: number) => Promise<void>; onCommit?: (id: string, name: string, destination: string, maxConnections: number) => void; onCancel: (id: string) => void; onClose: () => void }) {
  ...
  useEffect(() => { if (job) { setSource(job.source); if (!name) setName(job.name); if (destination === settings.defaultFolder) setDestination(job.destination); } }, [job?.id, job?.source, job?.name]);
